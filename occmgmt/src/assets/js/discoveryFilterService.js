...
import zoneFilterService from 'js/zoneFilterService';
...
var updatePersistedRecipeInfoInCache = function( productContextInfoUID ) {
    var filterData;
    var recipe;
    if( pciToFilterDataMap && pciToFilterDataMap.length > 0 ) {
        filterData = pciToFilterDataMap.filter( function( x ) {
...
var clearPersistentRecipeFromCache = function() {
    var activePCI = appCtxSvc.getCtx( _contextKey ).productContextInfo.uid;
    recipeHandler.clearPersistentRecipe( _contextKey, activePCI );
};
...
export let toggleCategoryLogic = function( toggleCategory, excludeCategoryToggleValue, updateAtomicData, searchState, recipeState, sharedData, occContext  ) {
    ...
    if( recipesData && recipesData[ 0 ] ) {
        ...
        if( recipeTermInTransientList && recipeTermInTransientList.length > 0 ) {
            ...
            updateTransientRecipeInCache( _.cloneDeep( recipesData[ 0 ].transientRecipes ) );
...
/**
    ...
    */
var updateTransientRecipeInCache = function( transientRecipe ) {
    var newRecipe = [];
...
let updateFilterInfo = function( searchState, soaResponseCategories, soaResponseFilterMap, recipe, processCategories ) {
    ...
    _.forEach( processedCategories, function( category, index ) {
        ...
        if( responseCategory ) {
            ...
            if( responseCategory.displayFilterText ) {
                ...
                processedCategories[ index ].isServerSearch = responseCategory.supportServerSearch;
            }
        }
    ...
    processCategoriesMetaDataForRendering( clonedFilterMap, processedCategories, processCategories );
    if( processCategories ) {
        initializeCategoryLogicCache( processedCategories, clonedRecipe );
    }
    ...
    updateCategoriesInfoCacheForCurrentPCI( _.cloneDeep( newSearchData.categories ), clonedFilterCategories, clonedFilterMap, clonedRecipe );
...
export let applyFilterInBulkMode = function( updateAtomicData, occContext ) {
    var transientRecipeInfo = recipeHandler.getTransientRecipeInfo( _contextKey );
...
var cloneCurrentRecipesIfNeeded = function() {
    var occMgmtCtx = appCtxSvc.getCtx( _contextKey );
    var productContextInfoUID = occMgmtCtx.productContextInfo.uid;
    var clonedRecipes = _.cloneDeep( recipeHandler.getPersistentRecipe( _contextKey, productContextInfoUID ) );
...
var updateRecipesInCache = function( sharedData ) {
    ...
    if( transientRecipes ) {
        ...
        var currentRecipes = recipeHandler.getPersistentRecipe( _contextKey, productContextInfoUID );
        ...
        updateTransientRecipeInCache( _.cloneDeep( transientRecipes ) );
    } else {
        updatePersistedRecipeInfoInCache( productContextInfoUID );
...
export let processInitialDelayedApplyPreference = function( result, activeViewSharedData, sharedData ) {
...
export let clearRecipe = function( searchState, recipeState, searchStateUpdater, occContext, sharedData ) {
    if( recipeState.isClearAll ) {
        ...
        if( recipe && recipe.length > 0 ) {
            ...
            var recipeToClear = _.cloneDeep( recipe[ 0 ] );
        ...
        if( searchState.autoApplyFilters ) {
            ...
            updateTransientRecipeInCache( updatedRecipeOnClear );
            // Update recipe state
...
var processRecipeFromSubsetResponse = function( recipes, subPanelContext ) {
    if( recipes ) {
        recipeHandler.setPersistentRecipe( _contextKey, subPanelContext.occContext.productContextInfo.uid, _.cloneDeep( recipes ) );
    }
...
export let initializeSearchStateFromSoaResponse = function( searchState, subPanelContext, searchFilterCategories, searchFilterMap, recipe, processCategories ) {
    let newSearchState = {};
...
export let modifySearchStateWithUpdatedFilters = ( searchState, recipeState, subPanelContext, updateAtomicData ) => {
    if( subPanelContext.occContext && subPanelContext.occContext.searchFilterMap && ( searchState.searchInProgress || searchState.categories ) ) {
        const newSearchState = initializeSearchStateFromSoaResponse( searchState, subPanelContext, subPanelContext.occContext.searchFilterCategories, subPanelContext.occContext.searchFilterMap,
            subPanelContext.occContext.recipe, true );
        revalidateIncludeExcludeCommandVisibility( subPanelContext.occContext.recipe );
        const newRecipeState = { ...recipeState };
        newRecipeState.recipe = _.cloneDeep( subPanelContext.occContext.recipe );
        newRecipeState.isClearAll = false;
...
/**
   ...
   * @param {Object} updateAtomicData - atomic data updater
   */
export const updateSharedActiveViewBasedOnPCI = ( sharedData, occmgmtContext, updateAtomicData ) => {
    let newSharedData;
    if( sharedData ) {
...
export let setContextKey = function( key ) {
    _contextKey = key;
    zoneFilterService.setContextKey( key );
};
