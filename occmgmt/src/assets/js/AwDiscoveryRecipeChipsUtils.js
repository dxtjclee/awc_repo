 ...
 */
function createChip( parent, numberOfChips, categoryName, filterDisplayName, internalCategoryName, internalFilterName, filterType, childrenChips, recipeTerm, recipeTermIndex, tooltip ) {
    let displayLabel;
...
export let getTooltip = function( recipeItem ) {
    ...
    if( selectedTerms( recipeItem.criteriaDisplayValue ).length <= 1 ) {
        ...
        var formattedTooltipValue = getRecipeValue( recipeItem.criteriaDisplayValue ).replace( /\^/g, ',\n' );
...
let getRecipeLabel = function( recipeItem, recipeDisplayName, isProximityTitle, nSelectedText ) {
    if ( recipeItem.criteriaType === 'Proximity' ) {
        const localTextBundle = localeService.getLoadedText( 'OccurrenceManagementSubsetConstants' );
        recipeDisplayName = localTextBundle.proximityTitle + ': ' + recipeDisplayName;
...
function getRecipeParentChip( recipeTerm, nSelectedText, recipeTermIndex ) {
    ...
    let tooltip = getTooltip( recipeTerm );
    return createChip( parent, numberOfChips, categoryName, recipeTermDisplayName, internalCategoryName, internalFilterName, filterType, childrenChips, recipeTerm, recipeTermIndex, tooltip );
}
