...
/**
  ...
  * @param {string} section - section ex.tc_xrt_AlignedParts
  * @returns {string} - mapped type to selected object in Pma0_Part_Design_Type preference
  */
export let getPreferredType = function( context, section ) {
    ...
    if ( prefValues ) {
        ...
        if ( !_.isEmpty( selectedObjectTypeName ) ) {
            for ( const prefValue of prefValues ) {
                ...
                // if section is CbaAlignPart then return Part or else return Design Associated with selected object
                // mapped in preference Pma0_Part_Design_Type
                if ( partDesignTypeArr.includes( selectedObjectTypeName ) ) {
                    if ( section === 'tc_xrt_AlignedParts' ) {
