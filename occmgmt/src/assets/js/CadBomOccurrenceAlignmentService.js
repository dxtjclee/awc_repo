...
import { skipAutoBookmarkForSplitView } from 'js/occmgmtSplitViewService';
...
let _makeGetAlignedObjectCallInOverviewTab = function( commandContext, mSelected, xrtSummaryContextObject ) {
    ...
    if( _isOverviewTabActiveInSWA( commandContext ) || _isOverviewTabActiveInPWA( commandContext ) ) {
        if( mSelected[ 0 ].uid === xrtSummaryContextObject.uid || !_isValidObjectToLaunchInCBA( xrtSummaryContextObject ) ) {
            return true;
        }

        let type;
        if( mSelected[ 0 ]?.modelType?.typeHierarchyArray?.indexOf( 'Awb0Element' ) > -1 ) {
            type = _.get( mSelected[ 0 ], 'props.awb0UnderlyingObjectType.dbValues[0]' );
        }

        return _isValidObjectToLaunchInCBA( mSelected[ 0 ], type );
    }
    return false;
};
 ...
 * @param {object} commandContext Command context object
 * @param {object} adaptObject adaptObject
 * @param {object} selectedEcnUid selectedEcnUid
 *
 */
export let setImpactAnalysisMode = function( commandContext, adaptObject, selectedEcnUid ) {
    commandContext.cbaContext.isImpactAnalysisMode = true;
    let impactAnalysis = {
        sourceTopItem: adaptObject,
        alignedTargetProviderInECN: 'Pma1AlignedTargetProvider',
        ECNForImpactAnalysis: cdmSvc.getObject( selectedEcnUid )
    };
    commandContext.cbaContext.ImpactAnalysis = impactAnalysis;

    appCtxSvc.updatePartialCtx( 'cadbomalignment.ImpactAnalysis', impactAnalysis );
    appCtxSvc.updatePartialCtx( 'cadbomalignment.ImpactAnalysis.isImpactAnalysisMode', true );
    appCtxSvc.updatePartialCtx( 'cadbomalignment.isImpactAnalysisMode', true );
 ...
 */
export let autoUpdateUnalignedIndicator = function() {
    ...
    for( let unalignmentInputsIndex = 0; unalignmentInputsIndex < unalignmentInputsCount; unalignmentInputsIndex++ ) {
        ...
        if( objectsToFindCount > 0 ) {
            if( !isRowSelectionInSrcAndTrg ) {
                ...
                findAlignedInfo[ cbaConstants.CBA_SRC_CONTEXT ] = Object.assign( srcContextTemp, processingResp.findAlignedInfo.CBASrcContext );
                findAlignedInfo[ cbaConstants.CBA_TRG_CONTEXT ] = Object.assign( trgContextTemp, processingResp.findAlignedInfo.CBATrgContext );
