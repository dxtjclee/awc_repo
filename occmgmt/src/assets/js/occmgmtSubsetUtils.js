...
export const getCSIDChainForSelected = ( selected, criteriaType ) => {
    var selectedObjCloneIds = [];
    _.forEach( selected, function( selection ) {
        var csid_path = objectToCSIDGeneratorService.getCloneStableIdChain( selection );
        if( occmgmtUtils.isMinimumTCVersion( 14, 3 ) && ( criteriaType === 'Proximity' || criteriaType === 'SelectedElement' ) ) {
            // if selection is from PWA and line is packed, append PackMaster to the target string
            var isPartiallySelected = acePartialSelectionService.isPartiallySelected( selection.uid );

            let awb0IsPacked = selection.props && selection.props.awb0IsPacked &&
            selection.props.awb0IsPacked.dbValues && selection.props.awb0IsPacked.dbValues[0] === '1';

            if( !isPartiallySelected &&  awb0IsPacked  ) {
                csid_path = csid_path.concat( ':PackMaster' );
            }
        }
...
export const updateReplayCSID = ( occContext ) => {
    var selectedObjCloneIds = getCSIDChainForSelected( occContext.selectedModelObjects );
