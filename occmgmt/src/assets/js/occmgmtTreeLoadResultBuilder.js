 ...
 */
const _updateTopNodeRelatedInformationInOutputStructure = ( treeLoadInput, treeLoadOutput, response, declViewModel ) => {
    ...
    _buildRootPathNodes( treeLoadInput.subPanelContext, treeLoadOutput, response, undefined, declViewModel );
...
var _updateVmNodeCreationStrategyForRootPathNodes = function( subPanelContext, vmNodeCreationStrategy, response, rootPathObjects, supportedFeatures ) {
...
// Determine if ths input rootPathNode is a placeholder node
function _isRootPathNodePlaceholder( rootPathNode, response, declViewModel ) {
 ...
 */
const _buildRootPath = ( subPanelContext, response, rootPathObjects, pciUid, treeLoadOutput, declViewModel ) => {
    /**
     * Determine the path to the 'root' occurrence IModelObject starting at the immediate 'parent' (t_uid)
     * object.
     */
     ...
     */
    var nextLevelNdx = -1;
    _updateVmNodeCreationStrategyForRootPathNodes( subPanelContext, treeLoadOutput.vmNodeCreationStrategy, response, rootPathObjects, treeLoadOutput.supportedFeatures );
    for( var ndx = rootPathObjects.length - 1; ndx >= 0; ndx-- ) {
        ...
        if( _isRootPathNodePlaceholder( currNode, response, declViewModel ) ) {
...
/**
 */
function _buildRootPathNodes( subPanelContext, treeLoadOutput, response, occurrenceId, declViewModel ) {
    ...
    if( cdmSvc.isValidObjectUid( occurrenceId ) ) {
        ...
        var rootPathNodes = _buildRootPath( subPanelContext, response, rootPathObjects, treeLoadOutput.productContextInfo.uid, treeLoadOutput, declViewModel );
...
const _shouldTopNodeBeDisplayed = ( treeLoadOput ) => {
    if ( treeLoadOput.supportedFeatures.Awb0ShowTopNodeFeature ) {
        return true;
    }
    return false;
};
...
const _buildTreeLoadOutputForFocusAction = ( treeLoadOutput, declViewModel, response, treeLoadInput ) => {
    treeLoadOutput.rootPathNodes = _buildRootPathNodes( treeLoadInput.subPanelContext, treeLoadOutput, response, undefined, declViewModel );
...
const updateOccContextValueWithProvidedInput = ( treeLoadOutput, occContextValue, valuesToCopyOrUpdateOccContext, valuesToResetAfterAction, copyDefaultValue ) => {
    if( valuesToCopyOrUpdateOccContext ) {
        let paramsToPopulateIfEmpty = [ 'recipe' ];
...
let _updateGlobalStateInformation = function( treeLoadOutput, finalOccContextValue, viewKey ) {
    ...
    let productInfo = awDataNavigatorService.getProductInfoForCurrentSelection( lastSelectedObject, finalOccContextValue );
...
const _buildTreeLoadOutputForInitializeAction = ( treeLoadInput, treeLoadOutput, declViewModel, response, contextState, uwDataProvider, subPanelContext ) => {
    ...
    _updateTopNodeRelatedInformationInOutputStructure( treeLoadInput, treeLoadOutput, response, declViewModel );
...
function _updateOccContextValueWithRootElementAndSelectionSource( occContextValue ) {
    ...
    var productInfo = awDataNavigatorService.getProductInfoForCurrentSelection( lastSelectedObject, occContextValue );
...
function _buildResponseForInitializeAction( treeLoadInput, treeLoadOutput, declViewModel, response, contextState, uwDataProvider, occContextValue, finalOccContextValue ) {
    ...
    if( !skipActiveTabUpdate ) {
        ...
        if ( !_.isEqual( swaContext.secondaryActiveTabId, sublocationState.secondaryActiveTabId )   ) {
...
let _populateObjectsToSelectInfo = function( newState, treeLoadOutput, objectsToFocusOn ) {
    if( objectsToFocusOn ) {
        treeLoadOutput.selectedModelObjects = objectsToFocusOn;
...
export let processGetOccurrencesResponse = function( treeLoadInput, response, contextState, declViewModel, uwDataProvider, soaInput ) {
    ...
    let treeLoadResult;
    let vmNodes = {};
    let newState = occmgmtGetOccsResponseService.getNewStateFromGetOccResponse( response, contextState, soaInput, uwDataProvider );
    ...
    if( !_.isUndefined( treeLoadOutput.vmNodeCreationStrategy ) && treeLoadOutput.vmNodeCreationStrategy.reuseVMNode ) {
        ...
        _.forEach( occContext.vmc.getLoadedViewModelObjects(), function( loadedVMObject ) {
    ...
    _populateObjectsToSelectInfo( newState, treeLoadOutput, response.objectsToFocusOn );
    ...
    if( _.isEqual( treeLoadInput.dataProviderActionType, 'initializeAction' ) ) {
        ...
        finalOccContextValue.lastDpAction = occContextValue.transientRequestPref.lastDpAction ? occContextValue.transientRequestPref.lastDpAction : 'initializeAction';
        ...
        if( !occContextValue.transientRequestPref.lastDpAction && !treeLoadOutput.requestPref.windowNotReused && !treeLoadOutput.supportedFeatures[ '4GStructureFeature' ]  ) {
            finalOccContextValue.lastDpAction = 'initializeActionWithWindowReuse';
        }
        ...
        if( !_.isEmpty( treeLoadOutput.topNodeOccurrence ) ) {
            ...
            if( _.isUndefined( treeLoadOutput.rootPathNodes ) && !_.isEmpty( response.parentChildrenInfos ) ) {
                ...
                _buildRootPathNodes( treeLoadInput.subPanelContext, treeLoadOutput, response, occurrenceId, declViewModel );
            }
        }
    }
    ...
    if( response.requestPref && response.requestPref.deltaTreeResponse &&
        ...
        var vmNodesMarkedForDeletion = _.filter( occContext.vmc.getLoadedViewModelObjects(), function( vmNode ) {
    ...
    _.each( treeRespProcessingHandlers, function( handler ) {
        if( handler.condition( soaInput, treeLoadInput, treeLoadOutput, occContext ) ) {
            handler.addOccContextAtomicDataForUpdate( response, finalOccContextValue, treeLoadInput, treeLoadOutput );
        }
    } );
