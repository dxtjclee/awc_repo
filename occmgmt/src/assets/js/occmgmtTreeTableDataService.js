...
function loadTreeNodePageWithProperties( newlyLoadedNodes, treeLoadResult, isFocusOccurrenceConfigured, uwDataProvider, declViewModel, contextState, subPanelContext ) {
    ...
    var nodesForPropertyLoadPage = treeNodes;
    // get scrolled page nodes
    if( !_.isUndefined( uwDataProvider.scrollPosition ) ) {
        nodesForPropertyLoadPage  = _getScrolledPageNodes( treeNodes, treeLoadResult, isFocusOccurrenceConfigured, uwDataProvider, contextState );
    ...
    if( nodesForPropertyLoadPage.length > occmgmtTreeTableBufferService.getInitialPropertyLoadPageSize() ) {
        nodesForPropertyLoadPage = nodesForPropertyLoadPage.slice( 0, occmgmtTreeTableBufferService.getInitialPropertyLoadPageSize() );
    }
    ...
    var propertyLoadContext = {
        clientName: 'AWClient',
        clientScopeURI: uwDataProvider.objectSetUri ? uwDataProvider.objectSetUri : treeLoadResult.productContextInfo.props.awb0ClientScopeUri.dbValues[0],
...
let _getTreeLoadResultBuiltFromGetOccResponse = function( treeLoadInput, soaInput, contextState, declViewModel, uwDataProvider, response ) {
    if( !declViewModel.isDestroyed() ) {
        ...
        if ( subPanelContext.provider.locationSupportsJitterFreePropLoad ) {
            if ( treeLoadInput.openOrUrlRefreshCase || _.isEqual( treeLoadInput.dataProviderActionType, 'nextAction' ) ||
                !_.isUndefined( uwDataProvider.scrollPosition ) && // scroll position is recorded
...
export let updateColumnPropsAndNodeIconURLs = function( propColumns, occurrenceNodes, contextState ) {
    ...
    _.forEach( propColumns, function( col ) {
        if( !col.typeName && col.associatedTypeName ) {
            ...
            var sortCriteria = appCtxSvc.getCtx( contextState.key ).sortCriteria;
...
function _updateContextStateOnUrlRefresh( treeLoadInput, contextState ) {
    ...
    if( treeLoadInput.openOrUrlRefreshCase ) {
        if( !_.isEmpty( contextState.occContext.currentState.incontext_uid ) ) {
            treeLoadInput.loadIDs.c_uid = contextState.occContext.currentState.incontext_uid;
 ...
 */
function _updateTreeLoadInputParameterForFilterOrConfigChangeAction( treeLoadInput, contextState ) {
    let occContext = contextState.occContext;
    ...
    if( occContext.currentState && occContext.currentState.incontext_uid && occContext.currentState.incontext_uid !== null
        && shouldClearInContextOverrideMode( contextState ) ) {
...
export let loadTreeTableData = function() {
    ...
    let contextState = {
        ...
        occContext: declViewModel.data.newOccContext ? declViewModel.data.newOccContext : subPanelContext.occContext.getValue()
    };
    ...
    _updateTreeLoadInputParameterForFilterOrConfigChangeAction( treeLoadInput, contextState );

    treeLoadInput.grid = grid;

    if( contextState.context.treeLoadingInProgress !== true ) {
        setTreeLoadingState( contextKey, true );
    }
...
export let loadOccurrencesWithFocusInTreeTable = function() {
    ...
    setTreeLoadingState( subPanelContext.contextKey, true );
    _populateSortCriteriaParameterForProvidedInput( treeLoadInput, contextState, newSortCriteria, soaInput );
...
export let loadTreeTableProperties = function() {
    ...
    if( expandRequests.loadTreePropertiesInBackgroundEnabled( subPanelContext.occContext ) ) {
        ...
        return AwPromiseService.instance.resolve( treeTableDataService.loadTreeTableProperties( arguments[ 0 ] ).then( function( response ) {
            expandRequests.loadTreePropertiesInBackground( commandContext );
            return response;
        } ) );
    }
...
export let populateTreeTableProperties = function( subPanelContext, uwDataProvider ) {
    let viewKey = appCtxSvc.ctx.aceActiveContext.key;
