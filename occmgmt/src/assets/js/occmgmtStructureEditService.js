...
/**
  ...
  * @param {Object}selection Selection to add
  ...
  */
function _buildOccContextValueForLoadNSelect( occContext, selections, lastSelection, elementToPCIMap, pci_uid ) {
    ...
    if ( isMultiSelectEnabled ) {
        ...
        if ( selectionsToAdd.length > 0 ) {
            occContextValue.selectedModelObjects = occContextValue.selectedModelObjects.concat( selectionsToAdd );
        }

        occContextValue.pwaSelection = [ ...occContextValue.selectedModelObjects ];
    } else {
        occContextValue.selectedModelObjects = selections;
...
export let loadAndSelectProvidedObjectInTree = function( occContext, selection, productContextInfo, nodeToExpandAfterFocus,
    nodeToExpand, updateVmosNContextOnPwaReset, getOccSoaInput, retainExpansionState ) {
            ...
            function( response ) {
                ...
                */
                if( updateVmosNContextOnPwaReset ) {
                    if( occContextValue.selectedModelObjects.length > 1 ) {
                        response.objectsToFocusOn = occContextValue.selectedModelObjects;
                        occmgmtUtils.updateValueOnCtxOrState( 'selectionsToModify', { elementsToSelect:occContextValue.selectedModelObjects, overwriteSelections:true }, occContext );
                    }
                    let value = {
                    ...
                    let nodeToExpandAfterFocusIsAlreadyExpanded = false;
                    vmoId = vmCollection.findViewModelObjectById( nodeToExpandAfterFocus );
                    if( vmoId !== -1 ) {
                        let nodeToExpandAfterFocusVmo = loadedVMOs[ vmoId ];
                        nodeToExpandAfterFocusIsAlreadyExpanded = nodeToExpandAfterFocusVmo.isExpanded;
                    }
                    let vmoToLookFor =  nodeToExpandAfterFocus && !nodeToExpandAfterFocusIsAlreadyExpanded ? lastSelection.props.awb0BreadcrumbAncestor.dbValues[0] : nodeToExpand;
