...
let _executeAlignmentCheckOnNodeExpansion = () => {
    ...
    if( srcCompareInfo && trgCompareInfo ) {
        ...
        let alignmentCheckInfo = _getAlignmentCheckInfoObject( CURRENT_LEVEL, CURRENT_LEVEL, sourceVMOs, targetVMOs,
            true, false, null, null, srcCompareInfo.element, trgCompareInfo.element );
    ...
    if( _.has( data, [ 'scope' ] ) && data.scope.subPanelContext ) {
        ...
        if( isCBAFirstLaunch ) {
            if( _sourceVMOs && _sourceVMOs.length > 0 && _targetVMOs && _targetVMOs.length > 0 ) {
                ...
                if( dataSetUID ) {
                    _executeAlignmentCheck( _getAlignmentCheckInfoObject( REUSE_SAVED_RESULTS, REUSE_SAVED_RESULTS, _sourceVMOs, _targetVMOs,
                        false, false, dataSetUID ) );
                    appCtxSvc.updatePartialCtx( CTX_PATH_DATASETUID, undefined );
                }
...
/**
 * Filter visibe uids which are already haveing alignment status
 ...
 */
let _filterVisibleUIDS = function( contextKey, visibleUids ) {
    let contextObj = _getExecutedAlignmentCheckInfo( contextKey );
    if( contextObj ) {
        let diffObj = contextObj.differences;
        let cachedUids = Object.keys( diffObj );
        return visibleUids.filter( x => cachedUids.indexOf( x ) === -1 );
    }
    return visibleUids;
};
...
/**
  ...
  * @param {boolean} isAddAllVisibleUids true to send all visible uids in compareContent else filtered uids will be sent
  * @returns {object} Compare info object
  */
let _getCompareInfo = function( contextKey, vmoList, depth, contextElement, isAddAllVisibleUids ) {
    ...
    if( !isAddAllVisibleUids ) {
        visibleUids = _filterVisibleUIDS( contextKey, visibleUids );
    }
...
let _createAlignmentCheckInput = function( alignmentCheckInfo ) {
    ...
    let sourceCompareInfo = _getCompareInfo( CBA_SRC_CONTEXT, sourceInfo.VMOs, sourceInfo.depth, sourceInfo.contextElement, isAddAllVisibleUids );
    let targetCompareInfo = _getCompareInfo( CBA_TRG_CONTEXT, targetInfo.VMOs, targetInfo.depth, targetInfo.contextElement, isAddAllVisibleUids );
...
/**
  ...
  * @returns {Object} The response object
  */
let _performAlignmentCheck = function( alignmentCheckInput, isOverwriteResponse ) {
...
/**
  ...
  */
export let executeAlignmentCheck = function( isRunInBackground, isScroll ) {
    let sourceVMOs = _getLoadedVMOjects( CBA_SRC_CONTEXT );
    let targetVMOs = _getLoadedVMOjects( CBA_TRG_CONTEXT );
    ...
    let alignmentCheckInfo = _getAlignmentCheckInfoObject( depthBackgroundInfo.depth, depthBackgroundInfo.depth, sourceVMOs, targetVMOs,
        true, backgroundOption, null, null, srcCtxElement, trgCtxElement );
 ...
 */
let _clearFindAlignedStatus = function( updatedUids ) {
...
/**
  ...
  * @param {List} updatedUids uids of the object participated in align/unalign operation
  */
let _clearAlignmentCheckStatus = function( updatedUids ) {
    appCtxSvc.updatePartialCtx( 'cbaContext.alignmentCheckContext.flags.alignmentCheckClicked', false );
...
/**
  ...
  */
export let clearAlignmentCheckStatus = function( dataProviderActionType, updatedUids ) {
