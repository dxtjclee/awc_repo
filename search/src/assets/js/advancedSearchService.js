...
export const  removeDuplicateEntries = ( lovDataInfo, preferredValues, choices, dataProvider, isNextValues ) =>{
    const lovPageSize = lovDataInfo?.responseData?.lovData?.filterData?.numberToReturn;
    const advSearchLovSize = lovPageSize ? lovPageSize : 25;
    var prefChoices = [];
    ...
    if( prefChoices.length > preferredValues.dbValues.length ) {
    ...
    } else if( prefChoices.length >= advSearchLovSize ) {
        lovDataInfo.lovEntries = [];
    } else if( prefChoices.length < advSearchLovSize ) {
        lovDataInfo.lovEntries = _.cloneDeep( prefChoices );
        prefChoices = [];
    }
