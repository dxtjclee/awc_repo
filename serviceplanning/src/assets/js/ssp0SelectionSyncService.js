...
let preparePath = function( response ) {
    if ( response ) {
        ...
        let tempPath = null;
        tempPath = Object.values( plains ).filter( plain => !foundNode.includes( plain ) );
        return ssp0BackingObjectProviderService.getDesignElementObject( tempPath ).then( ( result ) => {
            return result.ServiceData.plain;
        } );
    }
};
...
let getNodesToBeHighlighted = function( resultNodes ) {
    ...
    foundNodes.forEach( node => {
        nodesToBeHighlighted.push( node.uid );
    } );
    return ssp0BackingObjectProviderService.getDesignElementObject( nodesToBeHighlighted ).then( ( result ) => {
        return result.ServiceData.plain;
    } );
};
/**
...
export let selectNodeInSBOMTree = function( response, data ) {
    if ( response && response.resultInfo[0] && response.resultInfo[0].resultNodes[0] && response.resultInfo[0].resultNodes[0].foundNodes[0] ) {
        ...
        let nodeToBeHighlighted = null;
        let promise = getNodesToBeHighlighted( resultNodes );
        promise.then( ( result ) => {
            nodeToBeHighlighted = result;
            let promise1 = preparePath( response );
            promise1.then( ( result ) => {
                if ( result !== undefined ) {
                    const path = result;
                    const copyOfPath = [ ...path ];
                    expandMode = false;
                    let subscribeTreeNodesLoaded = null;
                    // Prepare path
                    for ( const node of copyOfPath ) {
                        if ( path.length > 0 && !expandMode ) {
                            // Subscribe event
                            if ( subscribeTreeNodesLoaded === null ) {
                                subscribeTreeNodesLoaded = eventBus.subscribe( 'sbomTreeDataProvider.treeNodesLoaded', function( eventData ) {
                                    if ( path.length > 0 ) {
                                        // take out 1st node
