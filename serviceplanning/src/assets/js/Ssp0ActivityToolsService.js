...
*
* @module js/Ssp0ActivityToolsService
*/
 ...
 */
export let loadToolsTreeTableColumns = async function( i18n ) {
 ...
 */
export const getActivityTools = function( response, data ) {
...
export let setTreeProperties = function( response, data ) {
    ...
    if ( response.modelObjects !== undefined ) {
        ...
        Object.values( modelObjects ).filter( modelObject => modelObject.props && modelObject.props.bl_line_name
            && modelObject.props.bl_occ_type && modelObject.props.bl_item_object_type && modelObject.modelType.typeHierarchyArray.includes( timeAnalysisConstants.TYPE_EQUIPMENT_PROCESS ) ).forEach(
            modelObject => {
...
let _getIconURL = function( parentObject ) {
    if ( parentObject.modelType.typeHierarchyArray.includes( timeAnalysisConstants.TYPE_EQUIPMENT_REVISION ) ) {
        return iconService.getTypeIconURL( timeAnalysisConstants.TYPE_RESOURCE_PROCESS );
    }
