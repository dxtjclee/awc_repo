...
/**
 * Activity Notices Functions
 *
 * @module js/ssp0ActivityNoticesService
 */
...
export let getCreateInput = async( data )=>{
    let Activity = appCtxSvc.getCtx( 'selectedActivity' );
    let ActivityRevID = Activity.props.me_cl_fnd0objectId.dbValue;
    let ActivityRev = cdm.getObject( ActivityRevID );
    let notices = [];
    const input = {
        primaryObjects: [ ActivityRev ],
        pref:
         {
             expItemRev: false,
             info:
 ...
 */
export let reloadActivityNoticesTree = function( dataProvider, selection ) {
    if ( dataProvider ) {
        dataProvider.columnConfigLoadingInProgress = false;
        eventBus.publish( 'activityNoticesTree.plTable.reload' );
    }
    if ( selection ) {
        const subscribeTablePageLoaded = eventBus.subscribe( 'activityNoticesTree.plTable.loadMorePages', function( eventData ) {
            eventBus.publish( 'Ssp0ActivityNoticesTree.selectVMOinTree' );
