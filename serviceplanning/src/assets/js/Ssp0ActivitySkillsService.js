...
/**
* Service to provide utilities to load SKills assigned to Activity
*
* @module js/Ssp0ActivitySkillsService
*/
 ...
 */
export let loadSkillsTreeTableColumns = async function( i18n ) {
 ...
 */
export const getActivitySkills = function( response, data ) {
...
export let setTreeProperties = function( response, data ) {
    ...
    if ( response.modelObjects !== undefined ) {
        ...
        Object.values( modelObjects ).filter( modelObject => modelObject.props && modelObject.props.bl_line_name
            && modelObject.props.bl_occ_type && modelObject.props.bl_item_object_type && modelObject.modelType.typeHierarchyArray.includes( timeAnalysisConstants.TYPE_SKILL ) ).forEach(
            modelObject => {
...
let _getIconURL = function( parentObject ) {
    if ( parentObject.modelType.typeHierarchyArray.includes( timeAnalysisConstants.TYPE_SKILL_REVISION_TYPE ) ) {
        return iconService.getTypeIconURL( timeAnalysisConstants.TYPE_SKILL_REVISION_TYPE );
    }
