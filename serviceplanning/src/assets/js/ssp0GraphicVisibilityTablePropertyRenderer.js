...
let _propertyRendererBasedOnPartsViewer = function( instanceId, vmo, containerElement, eventName ) {
    ...
    if ( instanceId === servicePlannerConstants.PARTS_VIEWER_INSTANCE_ID && viewerInstanceId && visWebInstanceProvider.getVisWebInstance( viewerInstanceId ) ) {
        const viewerInstance = visWebInstanceProvider.getVisWebInstance( viewerInstanceId );
        if ( viewerInstance !== undefined ) {
            visibilityState = viewerInstance.Viewer.getVisibilityState( vmo.uid );
        }
    }
    if ( instanceId === servicePlannerConstants.SBOM_VIEWER_INSTANCE_ID && viewerInstanceId ) {
        ...
        if ( viewerInstance !== undefined ) {
            ...
            visibilityState = viewerInstance.Viewer.getVisibilityState( bomLineUid[0].uid );
        }
    }
