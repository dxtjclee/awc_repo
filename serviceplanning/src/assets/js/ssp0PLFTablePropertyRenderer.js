...
let getPLFList = function() {
    ...
    soaSvc.post( 'Core-2007-06-DataManagement', 'expandGRMRelationsForPrimary', input ).then( function( response ) {
        ...
        let itr = 0;
        if ( outputArray.inputObject && outputArray.otherSideObjData && outputArray.otherSideObjData[0] && outputArray.otherSideObjData[0].otherSideObjects && outputArray.otherSideObjData[0].otherSideObjects[0] ) {
...
export let getPartList = function( vmos ) {
    ...
    if( vmos !== undefined && vmos[0] && vmos[0].props !== undefined && vmos[0].props.awb0UnderlyingObject !== undefined && vmos[0].props.awb0UnderlyingObject.dbValues !== undefined && vmos[0].props.awb0UnderlyingObject.dbValues[0] !== undefined ) {
        let topPart = cdm.getObject( vmos[0].props.awb0UnderlyingObject.dbValues[0] );
        let mselectedSBOM = [ topPart ];
        appCtxSvc.registerCtx( 'selectedSBOM', topPart );
        appCtxSvc.registerCtx( 'mselectedSBOM', mselectedSBOM );
    }
    const input =
    {
