...
/**
 * This method will return current selection object
 *
 * @returns {selected} will return selected object.
 */
const getSelectedObject = function(  ) {
    let selectedSecondary = appCtxSvc.getCtx( 'ssp0SelectedSkill' );
    let selected = [];
    for( const object of selectedSecondary ) {
        selected.push( cdm.getObject( object.props.bl_revision.dbValue ) );
    }
    return selected;
};

/**
 * This method will return current selection frequency
 *
 * @returns {selected} will return selected frequency.
 */
const getSelectedFrequency = function(  ) {
    let selectedSecondary = appCtxSvc.getCtx( 'ssp0SelectedFrequency' );
    let selected = [];
    for( const object of selectedSecondary ) {
        selected.push( cdm.getObject( object.props.bl_revision.dbValue ) );
    }
