    ...
    if( graphModel && node && node.model.nodeObject ) {
        ...
        if( edgeDirection ) {
            ...
            if( visibleEdges.length > 0 ) {
                ...
                const nodesToRemove = rbUtils.resolveConnectedGraph( graphModel, { edges: edges } );
                graph.removeItems( nodesToRemove.concat( edges ) );
            } else {
                actionStateValue = {
                    expandGraph: {
...
export let removeObjectsFromGraph = function( graphModel, nodeObjects ) {
    try {
        ...
        if( nodes.length > 0 ) {
            const nodesToRemove = rbUtils.resolveConnectedGraph( graphModel, { nodes: nodes } );
            graphModel.graphControl.graph.removeItems( nodesToRemove );
        }
    } catch ( ex ) {
        logger.error( ex );
    }
