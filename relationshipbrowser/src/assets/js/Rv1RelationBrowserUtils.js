...
const getNextLevelNodes = function( itemsToSkip ) {
    return function( node ) {
        ...
        const edgesToSkip = itemsToSkip && itemsToSkip.edges ? itemsToSkip.edges : [];
        const edges = node.getEdges();
        const visibleEdges = _.filter( edges, function( edge ) {
            return edge.isVisible() && !edgesToSkip.includes( edge );
        } );

        const nodesToSkip = itemsToSkip && itemsToSkip.nodes ? itemsToSkip.nodes : [];
        for( const edge of visibleEdges ) {
            if( edge.getSourceNode() === node ) {
                if( edge.getTargetNode().isVisible() && !nodesToSkip.includes( edge.getTargetNode() ) ) {
                    nextLevelNodes.push( edge.getTargetNode() );
                }
            } else if( edge.getTargetNode() === node ) {
                if( edge.getSourceNode().isVisible() && !nodesToSkip.includes( edge.getSourceNode() ) ) {
...
/**
 * Method to get orphan nodes by traversing connected graph from root node/s
 * @param {Object} graphModel Graph Model
 * @param {Object} itemsToSkip Graph Items to skip when evaluating connected graph in the format of { nodes: [], edges:[] }
 * @returns {Array} Orphan Nodes
 */
export let resolveConnectedGraph = function( graphModel, itemsToSkip ) {
    let orphanNodes = [];
    const graph = graphModel.graphControl.graph;

    const graphNodes = graph.getNodes();
    const visibleNodes = graph.getVisibleNodes();
    const rootNodes = _.filter( visibleNodes, rootNodeFilter );

    if( rootNodes && rootNodes.length > 0 ) {
        const connectedGraphNodes = graphPathsService.getConnectedGraph( rootNodes, getNextLevelNodes( itemsToSkip ) );
        orphanNodes = _.difference( graphNodes, connectedGraphNodes );
    }
    return orphanNodes;
};
...
export let updateRelationCounts = function( graphModel, legend ) {
    try {
        ...
        _.forEach( visibleNodes, function( node ) {
            var filteredEdgeCount = getFilteredCounts( legend, rootNodes, node );
