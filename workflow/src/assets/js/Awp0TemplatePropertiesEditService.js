...
var _getConditionTaskUpdateSOAInput = function( epmSetConditionHandler, data, inputData ) {
    ...
    var queryScope = data.vmo.props.queryScope.dbValue[0];
    var includeReplica = data.vmo.props.includeRepliaceTarget.dbValue[0];
    var queryName = data.vmo.props.savedQueries.dbValue[0];
    var queryType = data.vmo.props.queryAganist.dbValue[0];
    ...
    if( data.vmo.props.queryScope && data.vmo.props.queryScope.isEditable && epmSetConditionHandler ) {
        ...
        if( data.vmo.props.queryCondition.dbValue[0] === 'query' && queryType === 'sub-process' && ( data.vmo.props.queryAganist.value === 'task' || data.vmo.props.queryAganist.value === 'target' ) ||
    ...
    if( queryName !== '' && data.vmo.props.queryCondition.dbValue[0] === 'query' ) {
    ...
    if( conditionName !== '' && data.vmo.props.queryCondition.dbValue[0] === 'condition' ) {
...
var _getStatusHandlerUpdateSOAInput = function( epmCreateStatusHandler, releaseStatusProp, inputData ) {
    if( epmCreateStatusHandler && releaseStatusProp ) {
        var additionalDataMap = {};
        additionalDataMap[ '-status' ] = [ releaseStatusProp.dbValue[0] ];
        var object = {
