 ...
 */
export let getTargetObjects = function( panelContext ) {
    let targetObjects = [];
    let workflowProcessCandidates = appCtxSvc.getCtx( 'workflow_process_candidates' );
    // process the targetObjects from workflow_process_candidates only if the component in launched from NX, i.e. IsEmbeddedComponent is true
    if( workflowProcessCandidates && workflowProcessCandidates.IsEmbeddedComponent && workflowProcessCandidates.workFlowObjects && workflowProcessCandidates.workFlowObjects.length > 0 &&
        workflowProcessCandidates.workFlowObjects[0] ) {
        targetObjects = workflowProcessCandidates.workFlowObjects;
    } else {
        targetObjects = panelContext.targetObjects;
    }
