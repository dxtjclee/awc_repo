...
export let getDerivePropertiesNames = function( propToLoad ) {
    ...
    const updatedProperties = propToLoad.map((property) => {
...
/**
   ...
   */
export let populateCreatePanelPropertiesOnDerive = function( data, editHandlerIn, selectedChangeRevision, propToLoad ) {
...
function setValueOnCreatePanel( data, selectedChange, property, viewModelProp, updatedProps ) {
    ...
    // Set LOV or Array property with default values.
    if ( data[viewModelProp].hasLov || data[ viewModelProp ].isArray) {
        const propDisplayName = data[viewModelProp].propertyDisplayName;
        const propInternalName = data[viewModelProp].propertyName;
        const propType = data[viewModelProp].type;
        
        let viewProp = uwPropertyService.createViewModelProperty( propInternalName, propDisplayName, propType, selectedChange.props[ property ].dbValues, selectedChange.props[ property ].uiValues );
        
        uwPropertyService.setIsAutoAssignable( viewProp,  data[viewModelProp].isAutoAssignable );
        uwPropertyService.setLength( viewProp, data[viewModelProp].maxLength );
        if(data[viewModelProp].hasLov) {
            viewProp.lovApi = _.cloneDeep(data[viewModelProp].lovApi);
        }
        uwPropertyService.setHasLov( viewProp,  data[viewModelProp].hasLov );
        uwPropertyService.setIsArray( viewProp, data[ viewModelProp ].isArray  );
        uwPropertyService.setIsEnabled( viewProp, data[ viewModelProp ].isEnabled  );
        uwPropertyService.setIsEditable( viewProp, data[ viewModelProp ].isEditable  );
        uwPropertyService.setIsRequired( viewProp, data[ viewModelProp ].isRequired  );
        uwPropertyService.setIsNull( viewProp,  data[ viewModelProp ].isNull );
        uwPropertyService.setIsLocalizable( viewProp, data[ viewModelProp ].isLocalizable );

        data[viewModelProp] = _.cloneDeep(viewProp);
    }
    else {
        if(data[ viewModelProp ].type === "BOOLEAN") {
            data[ viewModelProp ].dbValue = propertyVal === "1" ? true : false;
        } else {
            data[ viewModelProp ].dbValue = propertyVal;
        }
        if(!_.isUndefined( selectedChange.props[ property ].uiValues)) {
