...
/**
 * This function is used to load 'part usage revision' objects.
 */
export let loadObjects = async function() {
    let sourceUid = appCtxService.ctx.state.params.srcPartUsgRevUID;
    let targetUid = appCtxService.ctx.state.params.targetPartUsgRevUID;
    if ( sourceUid !== null && targetUid !== null ) {
        let uids = [ sourceUid, targetUid ];
        await dmSvc.loadObjects( uids );
    }
};
    ...
    if( mergeCandidates.length > 0 ) {
        if( data.dataProviders && data.dataProviders.mergeCandidatesDataProvider ) {
            ...
            for( var count = 0; count < mergeCandidates.length; count++ ) {
                ...
                const isMergeConflict = mergeCandidate.mergeRelatedData.isMergeConflict && mergeCandidate.mergeRelatedData.isMergeConflict[0] === "true" ? true : false;
