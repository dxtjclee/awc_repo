...
function getChangeSummaryDataSOA( treeLoadInput, dataProvider, isGenealogy ) {
    ...
    var inputData = getChangeSummaryInputData( appCtxSvc.ctx, treeLoadInput, dataProvider, isGenealogy );
...
/**
  ...
  * @return {TreeLoadResult} A new TreeLoadResult object containing result/status information.
  */
export let processGetChangeSummaryDataResponse = function( dataForChangeSummaryTable, treeLoadInput, dataProvider, subPanelContext,rootObject ) {
        ...
        if ( dataProvider.name === 'mergeUsageChangesDataProvider' ) {
            if( isTopNode ) {
                ...
                if( dataProvider.name === 'mergeUsageChangesDataProvider' ) {
                    ...
                    if ( tableRows[index].supportingPrimaryVMO !== null &&
                        ( tableRows[index].supportingPrimaryVMO.uid === targetUsageRevID || tableRows[index].supportingPrimaryVMO.uid === srcUsageRevID ) ) {
                        tableRow = createTreeNode( tableRows[index], index, treeLoadInput, firstColumnName );
                        children.push( tableRow );
...
function createTreeNode( vmo, index, treeLoadInput, firstColumnName ) {
    ...
    if( treeLoadInput.parentNode && treeLoadInput.parentNode.props && treeLoadInput.parentNode.props.NAME
     && treeLoadInput.parentNode.props.NAME.currentValue ) {
        ...
        if (vmNode.modelType.typeHierarchyArray.indexOf('Ebm9EbomPartRevision') > -1) {
