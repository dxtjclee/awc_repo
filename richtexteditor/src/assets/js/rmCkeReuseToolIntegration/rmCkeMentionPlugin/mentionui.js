...
const handledKeyCodes = [
    ckeditor5ServiceInstance.keyCodes.arrowup,
    ckeditor5ServiceInstance.keyCodes.arrowdown,
    ckeditor5ServiceInstance.keyCodes.enter,
    ckeditor5ServiceInstance.keyCodes.tab,
    ckeditor5ServiceInstance.keyCodes.space,
    ckeditor5ServiceInstance.keyCodes.esc
];
 ...
 */
export default class MentionUI extends ckeditor5ServiceInstance.Plugin {
    /**
     * @inheritDoc
     */
     ...
     */
    static get requires() {
        return [ ckeditor5ServiceInstance.ContextualBalloon ];
    }
    ...
    constructor( editor ) {
         ...
         * @method
         */
        this._requestFeedDebounced = ckeditor5ServiceInstance.debounce( this._requestFeed, 100 );
    ...
    init() {
        ...
        editor.editing.view.document.on( 'keydown', ( evt, data ) => {
            ...
            if ( this._isUIVisible && isHandledKey( data.keyCode ) ) {
                data.preventDefault();
                evt.stop(); // Required for Enter key overriding.

                if ( data.keyCode === ckeditor5ServiceInstance.keyCodes.arrowdown ) {
                    this._mentionsView.selectNext();
                }

                if ( data.keyCode === ckeditor5ServiceInstance.keyCodes.arrowup ) {
                    this._mentionsView.selectPrevious();
                }

                if ( data.keyCode === ckeditor5ServiceInstance.keyCodes.enter || data.keyCode === ckeditor5ServiceInstance.keyCodes.tab ) {
                    this._mentionsView.executeSelected();
                }

                if ( data.keyCode === ckeditor5ServiceInstance.keyCodes.esc ) {
                    this._hideUIAndRemoveMarker();
                }
            } else if( idAttr && ( idAttr.indexOf( 'tables' ) !== -1 || idAttr.indexOf( 'figures' ) !== -1 || idAttr.indexOf( 'specContent' ) !== -1 ) ) {
                let handleKeyCodes = [
                    8, 9, 10, 11, 12, 13, 32, 45, 46,
                ...
                let unhandledKeyCodes = data.altKey || data.ctrlKey;
                let notAllowed = handleKeyCodes.includes( data.keyCode );
                if ( !unhandledKeyCodes && notAllowed ) {
                    data.preventDefault();
                    evt.stop();
                }
            }
        ...
        for ( const mentionDescription of feeds ) {
            ...
            if ( !isValidMentionMarker( marker ) ) {
                 ...
                 */
                throw new ckeditor5ServiceInstance.CKEditorError(
    ...
    _createMentionView() {
        ...
        this._items = new ckeditor5ServiceInstance.Collection();
    ...
    _requestFeed( marker, feedText ) {
        ...
        feedResponse
            ...
            .catch( error => {
                 ...
                 */
                console.warn( ckeditor5ServiceInstance.attachLinkToDocumentation( 'mention-feed-callback-error: Could not obtain mention autocomplete feed.' ) );
            } );
    }
    ...
    _setupTextWatcherForFeed( marker, minimumCharacters ) {
        ...
        const watcher = new ckeditor5ServiceInstance.TextWatcher( editor.model, createTestCallback( marker, minimumCharacters ) );
    ...
    _renderItem( item, marker ) {
        ...
        if ( !view ) {
            const buttonView = new ckeditor5ServiceInstance.ButtonView( editor.locale );
