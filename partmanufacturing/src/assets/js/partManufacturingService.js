...
var _toolActCustomUserAttrDisplayNameMap = {};
...
function getTypesForAddResource() {
    var resLength = appCtxSvc.ctx.preferences.MRMCreateDefaultResourceTypes ? appCtxSvc.ctx.preferences.MRMCreateDefaultResourceTypes.length : 0;
    var resTypesStr = '';
    if( resLength > 0 ) {
        for( var idx = 0; idx < resLength; idx++ ) {
            resTypesStr += appCtxSvc.ctx.preferences.MRMCreateDefaultResourceTypes[ idx ];
            if( idx < resLength - 1 ) {
                resTypesStr += ',';
            }
        }
    } else {
        // Use resource type Mfg0MEResource if pref is not MRMCreateDefaultResourceTypes found
    ...
    return dataManagementSvc.loadObjects( allChildUids ).then(
        function() { // eslint-disable-line no-unused-vars
            ...
            _.forEach( allChildNodes, function( childNode ) {
                var mo = cdm.getObject( childNode.uid );
                var vmo = viewModelObjectSvc.constructViewModelObjectFromModelObject( mo, 'EDIT' );
                if( mo.modelType.typeHierarchyArray.indexOf( 'Awb0Element' ) > -1 ) {
                    var awb0UnderlyingType = vmo.props.awb0UnderlyingObjectType ? vmo.props.awb0UnderlyingObjectType.dbValues[0] : undefined;

                    if( awb0UnderlyingType && awb0UnderlyingType !== 'MENCMachining Revision' && vmo.props.awb0OccType ) {
                        var occTypeProp = vmo.props.awb0OccType;
                        uwPropertySvc.setHasLov( occTypeProp, true );
                        uwPropertySvc.updateViewModelProperty( occTypeProp );
                        vmo.props.awb0OccType = occTypeProp;
                    }
                }
                vmoObjs.push( vmo );
            } );
...
function _buildActivityStructure( parentNode, deferred, treeLoadInput, columnConfigInput, inflateProp, subPanelContext ) {
    ...
    columnConfigInput.clientScopeURI = columnConfigInput.clientScopeURI ? columnConfigInput.clientScopeURI : 'Pm1Activities';
    ...
    treeLoadInput.parentElement = parentNode && parentNode.levelNdx > -1 ? parentNode.id : 'AAAAAAAAAAAAAA';
    ...
    return soaSvc.postUnchecked( 'Internal-AWS2-2023-06-Finder', 'performSearchViewModel5', soaInput ).then(
        function( response ) {
            ...
            if( response.columnConfig.columnConfigId === 'ncActivitiesColConfig' ) {
                //update the display name of user data attributes based on preference MPP_ToolActivity_CustomUserData
                response.columnConfig = updateNCActivitiesColConfig( response.columnConfig );
            }
            deferred.resolve( {
...
export let loadAttachmentsData = function( dataProvider, columnConfigInput, saveColumnConfigData, inflateProp, subPanelContext, searchSortCriteria ) {
    var selectedMO = subPanelContext.provider.baseSelection;
    ...
    return soaSvc.postUnchecked( 'Internal-AWS2-2023-06-Finder', 'performSearchViewModel5', inputData ).then(
        function( response ) {
            if( response.searchResultsJSON ) {
                response.searchResults = JSON.parse( response.searchResultsJSON );
                delete response.searchResultsJSON;
...
export let loadResourcesData = function( declViewModel, dataProvider, columnConfigInput, saveColumnConfigData, inflateProp, subPanelContext, searchSortCriteria ) {
    var partMfgCtx = appCtxSvc.getCtx( 'PartMfg' );
...
export const updateStateInfoToUrl = ( searchState, searchStateUpdater ) => {
    ...
    if( searchState.name === 'resourcesState' && commandId === 'Pm1OccTypeFilter' ||
        ...
        let skipURL = undefined;
        updateURL( searchStateProp, searchState, { searchState: searchStateUpdater[ searchState.name ] }, skipURL );
    }
};

export const updatePWASelectionInfo = ( searchState, selectionInfo, dataProvider1, dataProvider2 ) => {
    let searchData = { ...searchState.value };
    if( selectionInfo.selected && selectionInfo.selected.length > 0 ) {
        searchData.pwaSelection = selectionInfo.selected;
        dataProvider1.selectNone();
        dataProvider2.selectNone();
        searchState.update( { ...searchData } );
    } else {
        if( isDeselectionFromSameTable( searchState, selectionInfo ) ) {
            searchData.pwaSelection = selectionInfo.selected;
            searchState.update( { ...searchData } );
        }
    }
...
export const updateResourcesTableSelection = ( dataProvider, pwaSelection ) => {
    dataProvider.selectionModel.setSelection( pwaSelection );
    return dataProvider;
};
export let updatePWASelection = function( selection, searchState ) {
    let searchData = { ...searchState.value };
    searchData.pwaSelection = selection;
    searchState.update( { ...searchData } );
};
...
export let updateNCActivitiesColConfig = function( columnConfig ) {
    let actColumnConfig = _.clone( columnConfig );
    if( _.isEmpty( _toolActCustomUserAttrDisplayNameMap ) ) {
        return prefSvc.getStringValues( 'MPP_ToolActivity_CustomUserData' ).then( ( preferenceValues ) => {
            if( Array.isArray( preferenceValues ) && preferenceValues.length > 0 ) {
                preferenceValues.forEach( ( userDataEntry ) => {
                    let [ propName, momVar, displayName ] = userDataEntry.split( ':' );
                    _toolActCustomUserAttrDisplayNameMap[ propName ] = displayName;
                } );
                _.forEach( actColumnConfig.columns, function( column ) {
                    if( column.propertyName.startsWith( 'pm0UserData' ) ) {
                        column.displayName = _toolActCustomUserAttrDisplayNameMap[ column.propertyName ];
                    }
                } );
            }
            return actColumnConfig;
        } );
    }
    _.forEach( actColumnConfig.columns, function( column ) {
        if( column.propertyName.startsWith( 'pm0UserData' ) ) {
...
/**
 * checks if last selection and deselection is from same table
 * @param {searchState} searchState
 * @param {selectionInfo} selectionInfo
 * @returns {verdict} a verdict
 */
function isDeselectionFromSameTable( searchState, selectionInfo ) {
    var verdict = false;
    if( searchState.pwaSelection && searchState.pwaSelection.length > 0 ) {
        var prevSelection = cdm.getObject( searchState.pwaSelection[0].uid );
        verdict =  prevSelection.modelType.typeHierarchyArray.indexOf( 'MECfgLine' ) > -1 && selectionInfo.id === 'gridDataProvider' ||
        prevSelection.modelType.typeHierarchyArray.indexOf( 'Awb0Element' ) > -1 && selectionInfo.id === 'resourcesDataProvider' ||
        prevSelection.modelType.typeHierarchyArray.indexOf( 'Dataset' ) > -1 && selectionInfo.id === 'attachmentsDataProvider';
    }
    return verdict;
}
...
export default exports = {
    ...
    updateResourcesTableSelection,
    updatePWASelection,
    loadResourcesColumns,
    downloadActivityFiles,
    updateNCActivitiesColConfig
};
