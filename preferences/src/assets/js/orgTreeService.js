...
import awTableTreeSvc from 'js/published/splmTablePublishedTreeService';
 ...
 /**
    ...
    * UID for Group->Subgroup->Role->User
    org-tree-Organization-ADM_GROUP5->Group(ADM_GROUP5)
    org-tree-Organization-ADM_GROUP5-ADM_SUBGROUP1->For Subgroup(ADM_SUBGROUP1)
    org-tree-Organization-ADM_GROUP5-ADM_SUBGROUP1-ADM_ROLE7->For Role(ADM_ROLE7)
    org-tree-Organization-ADM_GROUP5-ADM_SUBGROUP1-ADM_ROLE7-ADM_USER9 (adm_user9)->For User(ADM_USER9)
 */
   var generateID = function( parentName, childName, parentId ) {
    return parentId && parentId.startsWith( 'org-tree-' ) ? parentId + '-' + childName : 'org-tree-' + parentName + '-' + childName;
};
 ...
 var _addGroupToHierarchy = function( parentNode, groupName, orgObject, index, groupsToAdd ) {
     ...
     var vmNode = _createViewModelTreeNode( orgObject.group, parentNode.node.levelNdx + 1, //
         index, parentNode.node.displayName, parentNode.node.id, parentNode.node.fullName, parentNode.node.hierarchy, parentNode.node );
 ...
 var _addSubgroupsToHierarchy = function( parent, vmNode, groupsToAdd ) {
     ...
     for( var i = 0; i < children.length; i++ ) {
         childNode = _createViewModelTreeNode( children[ i ].group, vmNode.levelNdx + 1, i, vmNode.displayName, vmNode.id, vmNode.fullName, vmNode.hierarchy, vmNode.node );
 ...
 var checkAddNode = function( _group, adminGroups ) {
     var addNode = true;
     var childGroups = _group.group.props.object_string.dbValues[ 0 ].split( '.' );
     var index = childGroups.length - 1;
     var groupAdminIndex = adminGroups.length - 1;
     ...
     while( index >= 0 && groupAdminIndex >= 0 && addNode === true ) {
         ...
         if( childGroups[ index ] !== adminGroups[ groupAdminIndex ] ) {
             addNode = false;
         }
         index--;
         groupAdminIndex--;
     }
     return { addNode : addNode, index : index };
 };
 ...
 var _buildGroupsAndSubgroups = function( deferred, treeLoadInput, data, expandingNode ) {
     ...
     if( !_.isUndefined( children ) && children.length > 0 &&  ( ctxfilteringOrgTree === true  || //
       expandingNode.fullExpansion === true ) ) {
        ...
        var treeLoadResult = awTableTreeSvc.buildTreeLoadResult( newctxtreeLoadInput, newctxorgTreeData[ treeLoadInput.parentNode.hierarchy ].children, false, true, true, null );
         ...
         return soaService.postUnchecked( 'Internal-Core-2007-01-DataManagement', 'getOrganizationInformation', //
             ...
             .then( function( response ) {
                 ...
                 for( var i = 0; i < rootGroups.length; i++ ) {
                     ...
                     if ( ctxgroupAdmin ) {
                         var res = checkAddNode( rootGroups[i], groupAdminGroups );
                         addNode = res.addNode;
                         index = res.index;
                     }
                     if( addNode === true && rootGroups[ i ].parent.className === 'unknownClass' ) {
                 ...
                 for( var i = 0; i < subGroups.length; i++ ) {
                     ...
                     if( addNode === true ) {
                         // this is a subgroup
                         var subgroupName = subGroups[ i ].group.props.object_string.dbValues[ 0 ];
                         ...
                         if( _.isUndefined( parentNode ) ) {
                             ...
                             groupsToAdd[ parentId ].push( subGroups[ i ] );
                         } else {
                             // parent already exists, so add the subgroup to the hierarchy
                             data = _addGroupToHierarchy( parentNode, subgroupName, subGroups[ i ], index, groupsToAdd );
                             vmNode = data.vmNode;
                             parent = data.parent;
 ...
 /**
    ...
    * @returns {Promise} resolved promise
    */
 export let _buildRoleAndUserTreeStructure = function( deferred, treeLoadInput, data, expandingNode ) {
     var children = expandingNode.children;
     var ctxtreeLoadInput = appCtxService.getCtx( 'treeLoadInput' );
     let newctxtreeLoadInput = { ...ctxtreeLoadInput };
     var ctxfilteringOrgTree = appCtxService.getCtx( 'tcadmconsole.preferences.filteringOrgTree' );
     ...
     if( !_.isUndefined( children ) && children.length > 0 &&  ( ctxfilteringOrgTree === true || //
        ...
        var treeLoadResult = awTableTreeSvc.buildTreeLoadResult( newctxtreeLoadInput, children, false, true, true, null );
         ...
         if( treeLoadInput.parentNode.type === 'Group' ) {
             groupName = treeLoadInput.parentNode.fullName;
         } else if( treeLoadInput.parentNode.type === 'Role' ) {
             roleName = treeLoadInput.parentNode.fullName;
             groupName = treeLoadInput.parentNode.parent.fullName;
         }
             ...
             .then( function( response ) {
                 ...
                 if( response.groupElementMap ) {
                     ...
                     for( var i = 0; i < groupMap.length; i++ ) {
                         ...
                         if( treeLoadInput.parentNode.type === 'Group' ) {
                            ...
                            if( treeLoadInput.parentNode.parent.fullName === groupInMap.group.props.object_full_name.dbValues[ 0 ] ) {

                                _.forEach( groupInMap.members, member=>{

                                    if( member.role.props.object_string.dbValues[ 0 ] === treeLoadInput.parentNode.fullName ) {

                                        children = children.concat( groupInMap.members[ 0 ].members );
                 ...
                 for( var j = 0; j < children.length; j++ ) {
                     var id = generateID( newctxtreeLoadInput.parentNode.fullName, children[ j ].type === 'Role' ? children[ j ].props.object_string.dbValues[ 0 ] : children[ j ].props.user.uiValues[
                         0 ], newctxtreeLoadInput.parentNode.id);
                     if( !_.some( expandingNode.children, [ 'id', id ] ) ) {
                         var vmNode = _createViewModelTreeNode( children[ j ], newctxtreeLoadInput.parentNode.levelNdx + 1, j, newctxtreeLoadInput.parentNode.displayName, //
                             newctxtreeLoadInput.parentNode.id, newctxtreeLoadInput.parentNode.fullName, newctxtreeLoadInput.parentNode.hierarchy, newctxtreeLoadInput.parentNode );
 ...
 var _createViewModelTreeNode = function( orgObject, levelNdx, childNdx, parentDisplayName, parentId, parentFullName, parentHierarchy, parentNode ) {
     ...
     var id = orgObject.type !== 'Site' ? generateID( parentFullName, displayName, parentId ) : orgObject.type;

    var vmNode = awTableTreeSvc.createViewModelTreeNode( id, orgObject.type, displayName, levelNdx, childNdx, '' );
 ...
 export default exports = {
     ...
     doSearch,
     scrollToSelected,
     _buildRoleAndUserTreeStructure
 };
