...
var getIndexOfSelectedPrefNameAndLoc = function( selectedPrefName, selectedPrefLoc, matchedPreferences ) {
    for( var i = 0; i < matchedPreferences.length; i++ ) {
        var objHdrFtr = cdm.getObject( matchedPreferences[ i ].uid );
        if( selectedPrefName === objHdrFtr.props.prf1Name.dbValues[0]  && selectedPrefLoc === objHdrFtr.props.prf1Location.uiValues[0] ) {
            return i;
        }
    }
    return -1;
...
export let selectNewPrefRow = function( newPref, newPrefLoc, vmData, selectedPreference ) {
    ...
    if( !_.isUndefined( newPref ) && _.isUndefined( newPrefLoc ) && vmData.searchResults.objects.length > 0 && newPref !== cachedNewPrefName ) {
        ...
        var selectedPreferenceLoc = selectedPreference.props.prf1Location.uiValue;
        ...
        if( selectedPreferenceLoc === _localTextBundle.None && index === -1 ) {
            // When None location preference is given some value.
            index = getIndexOfSelectedPrefNameAndLoc( selectedPreference.props.prf1Name.dbValue, _localTextBundle.Site, vmData.searchResults.objects );
            selectionModel.setSelection( vmData.searchResults.objects[ index ] );
        }
    }
};
...
export const formatDate = function( date_values ) {
    var months = [ 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec' ];
    for( let i = 0; i < date_values.length; i++ ) {
        var temp_date = new Date( date_values[ i ] );
        date_values[ i ] = temp_date.getDate() + '-' + months[temp_date.getMonth()] + '-' + temp_date.getFullYear() + ' ' + temp_date.getHours() + ':' + temp_date.getMinutes();
    }
    return date_values;
};
