...
export let preferenceInfoReveal = function( selectedRow, prefCtx ) {
    ...
    if( !prefInstance.fnd0IsMultiValue && ( prefInstance.fnd0PreferenceValues[0] === null || prefInstance.fnd0PreferenceValues[0] === '' ) ) {
        prefInstance.fnd0PreferenceValues = [];
    }
...
/**
   * Get display value for 'location' label on Info panel.
   * @param {boolean} value location string
   *
   * @return {String} display value for location
   */
export let getDisplayValueForLocation = function( value ) {
    var idx = value.indexOf( ' (' );
    var loc = value.substring( 0, idx === -1 ? value.length : idx );
    return localeService.getLoadedText( 'preferenceMessages' )[ loc ] + ( idx === -1 ? '' : value.substring( idx, value.length ) );
};
...
export let getVMPropertyType = function( isArray, valueType ) {
    if( valueType === 'Integer' ) {
        ...
        return isArray === true ? 'DATEARRAY' : 'DATETIME';
    }
...
/**
   ...
   */
export let startEditPreference = function( ) {
    var ctxeditInProgress = appCtxSvc.getCtx( 'tcadmconsole.preferences' );
    let newctxEdit = { ...ctxeditInProgress };
...
/**
   ...
   * @param {Object} prefCtx - admin console context
   */
export let cancelPreferenceEdits = function() {
    var prefEditHandler = editHandlerService.getEditHandler( 'PREF_EDIT_CONTEXT' );
    if( prefEditHandler !== null ) {
...
export let updatePreference = function( vmData, selectedPreference, prefCtx ) {
    ...
    assert( description !== undefined && description !== '', 'Description can not be empty.' );
    ...
    assert( vmData.preferenceInfo.props.fnd0Location.dbValue === selectedLocation, 'Location of preference instance can not be changed.' );

    if( selectedPreference.props.prf1Type.dbValue === 'Date' && prefValues.length > 0 ) {
        // SOA expects date in "DD-MMM-YYYY HH:MM" format. Date-time widget returns date in "YYYY-MM-DD HH:MM" format for non-english locales.
        if( selectedPreference.props.prf1Array.dbValue === false && ( prefValues[0] === '' || prefValues[0] === 'NaN-undefined-NaN NaN:NaN' ) ) {
            prefValues = [];
        } else{
            prefValues = prefService.formatDate( prefValues );
        }
    }
