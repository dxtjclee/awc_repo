...
export let loadTreeTableData = function( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, isInitialLoad ) {
    ...
    if( searchInput.columnFilters && searchInput.columnFilters.length > 0 ){
        // update the 'isColumnFilteringApplied' flag to 'true', if the column filter applied
        appCtxSvc.updatePartialCtx( 'isColumnFilteringApplied', true );
    }
    else{
        // update the 'isColumnFilteringApplied' flag to 'false', if the column filter removed or not applied
        appCtxSvc.updatePartialCtx( 'isColumnFilteringApplied', false );
    }
...
var _buildTreeTableStructure = function( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput, isInitialLoad, declViewModel, subPanelContext ) {
    ...
    return getTableSummary( soaSearchInput, columnConfigInput, saveColumnConfigData ).then( function( response ) {
        ...
        if( treeLoadInput.isTopNode && subPanelContext.treeTableData ) {
            subPanelContext.treeTableData.update( { ...subPanelContext.treeTableData.getValue(), rootNode: treeLoadResult.parentNode } );
        }
...
export let processEvents = function( soaResponse, data, subPanelContext ) {
    ...
    let planEventMap = {};
    let eventObjs = subPanelContext.timelineData.getValue().eventObjects ? subPanelContext.timelineData.getValue().eventObjects : [];
