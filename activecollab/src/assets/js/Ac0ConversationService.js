        ...
        for ( var i = 0; i < sourceTags.length; i++ ) {
            if( sourceTags[i].theObject ) {
                ...
                if( typeof underlyingObj !== 'undefined' ) {
                    ...
                    if( sourceTags[i].theObject.modelType && sourceTags[i].theObject.modelType.parentTypeName && sourceTags[i].theObject.modelType.parentTypeName === 'RuntimeBusinessObject' ) {
                        var underlyingObj = adapterService.getAdaptedObjectsSync( [ sourceTags[i].theObject ] );
                        if ( underlyingObj && underlyingObj[0].uid ) {
                            var tempUid = underlyingObj[0].uid;
                        }
                        tmpObj = {
                            uid: tempUid,
                            type: sourceTags[i].theObject.type
...
export let onObjectTabSelectionChange = function( selObjVm, selectionData, sharedData ) {
    ...
    if( convCtx.currentSelectedSnapshot && selObjVm.dbValue && selObjVm.dbValue !== '' && convCtx.snapshotEntryPoint  && convCtx.snapshotEntryPoint !== 'SnapshotProductGallery' ) {
    ...
    if( selectionData && selectionData.value && selectionData.value.selected[0].props &&
        selectionData.value.selected[0].props.awb0UnderlyingObject && selectionData.value.selected[0].props.awb0UnderlyingObject.dbValues[0] !== null ) {
...
export let modifyConversations = function( vmData, discussionsLoadedState ) {
    ...
    Promise.all( commentPromiseArray ).then( ( values ) => {
        for( var ii = 0; ii < values.length; ii++ ) {
            ...
            if( typeof ac0ConvCtx.convDP === 'undefined' || typeof ac0ConvCtx.convDP.viewModelCollection === 'undefined' || ac0ConvCtx.convDP.viewModelCollection === null ) {
...
export let replyBoxAction = function( discussionItem, cke, discussionData, searchState ) {
    ...
    if( convUtils.isDiscussionSublocation() ) {
        eventBus.publish( 'replyBoxAction.resetPWAEvent' );
    }
    return deferred.promise;
};
