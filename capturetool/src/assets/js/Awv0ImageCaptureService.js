...
export let populateCaptureList = function( imageCapturePanelData, viewerContextData ) {
    ...
    if( !imageCapturePanelDataValue.isImageCaptureContextPinned ) {
        ...
        if( imageCapturePanelDataValue.isImageCaptureContextPinned ) {
            ...
            } else if( getViewerContextData() ) {
                let selection = getCurrentSelection( getViewerContextData() )[ 0 ];
...
let _subscribeForImageCapturePanelCloseEvent = function( ) {
    if( _imageCaptureToolAndInfoPanelCloseEventSubscription === null ) {
        _imageCaptureToolAndInfoPanelCloseEventSubscription = eventBus.subscribe( 'appCtx.register', function(
            ...
            if( eventData.name === 'sidenavCommandId' ) {
                if( eventData.value ) {
                    if( eventData.value === 'Awp0Markup' || eventData.value === 'Awp0MarkupEditMain' ||
                        eventData.value === 'Awp0MarkupMain' || eventData.value === 'Awv0CaptureGallery' ) {
...
export let createImageCapture = function( captureName, captureDesc, imageCapturePanelData, viewerContextData ) {
    ...
    viewerContextData.getImageCaptureManager().captureImage( contextObjectUid, captureName, captureDesc ).then( function( resultUid ) {
 ...
 */
let setActiveTab = ( selectedTab, tabsModel, setDefaultTab, tcSessionData, viewerContextData ) => {
    ...
    if( setDefaultTab && viewerContextData && isTcVersion133OrHigher ) {
