 ...
 *
 * @module js/Crt1CreateStudySibilingService
 */
...
export let initCreateStudyPanel = function( openedObject, selectedScopeObj, i18n, dataDisplayedType ) {
    ...
    parentOfSelection = cdm.getObject( parentContract.props.crt0ParentVldnContract.dbValues[0] );
    if ( parentContract.modelType.typeHierarchyArray.indexOf( 'IAV0TestRunRevision' ) > -1 && parentOfSelection.type === 'IAV0TestStudyRevision' ) {
        selType = {
            dbValue: 'IAV0TestRun'
        };
        displayedType.propertyDisplayName = i18n.Crt1AddTestEventPanelTitle;
        panelVisibility = true;
    } else if ( parentContract.modelType.typeHierarchyArray.indexOf( 'Crt0RunRevision' ) > -1 && ( parentOfSelection.type === 'Crt0SimStudyRevision' || parentOfSelection.type === 'Crt0StudyRevision' ) ) {
        selType = {
            dbValue: 'Crt0Run'
        };
...
export let getPopulationList = function( parentContract ) {
    ...
    var object = cdm.getObject( parentContract.props.crt0ParentVldnContract.dbValues[0] );
    if ( object ) {
        var populationValueDisplayStr = object.props.object_name.dbValues[0];
        ...
        dmSvc.getPropertiesUnchecked( objects, propNames ).then( function() {
            var childStudyUids = object.props.crt0ChildrenStudies.dbValues;
            for ( var i = 0; i < childStudyUids.length; i++ ) {
                var object1 = cdm.getObject( childStudyUids[i] );
                if ( populationInput.indexOf( object1 ) === -1 ) {
                    populationInput.push( object1 );
                }
            }
            eventBus.publish( 'CreateStudy.convertStudiesToList', populationInput );
        } );
        return populationValueDisplayStr;
    }
};
...
export let ensureStudyTypesLoadedJs = function( getStudySubtypesResponseOutput, openedObject, selectedScopeObj ) {
    ...
    if ( promise ) {
        promise.then( function() {
            ...
            var isRunSelected = false;
            var isTESelected = false;
            if ( selectedScopeObj && selectedScopeObj.props && selectedScopeObj.props.crt0ParentVldnContract && selectedScopeObj.props.crt0ParentVldnContract.dbValues ) {
                var selectedParent = cdm.getObject( selectedScopeObj.props.crt0ParentVldnContract.dbValues[0] );
            }
            if ( selectedScopeObj ) {
                isStudySelected = selectedScopeObj.modelType.typeHierarchyArray.indexOf( 'Crt0StudyRevision' ) > -1;
                isRunSelected = selectedScopeObj.modelType.typeHierarchyArray.indexOf( 'Crt0RunRevision' ) > -1;
                isTESelected = selectedScopeObj.modelType.typeHierarchyArray.indexOf( 'IAV0TestRunRevision' ) > -1;
            }
            for ( var i = 0; i < displayableStudyTypes.length; i++ ) {
                ...
                } else if ( isRunSelected && ( selectedParent.type === 'Crt0SimStudyRevision' || selectedParent.type === 'Crt0StudyRevision' ) ) {
                    modelType = cmm.getType( displayableStudyTypes[i] );
 ...
 */
export let handleTypeSelectionJs = function( data, getStudySubtypesResponseOutput ) {
...
export let createStudyInit = function( data, openedObject, editHandler ) {
    ...
    var crt0ParentVldnContract = data.parentContract.props.crt0ParentVldnContract.dbValues[0];
    var crt0Population;
    ...
    if( createInputs[ 0 ] && createInputs[ 0 ].createData && createInputs[ 0 ].createData.compoundCreateInput && createInputs[ 0 ].createData.compoundCreateInput.revision &&
        ...
        for( let entry of Object.entries( newProperty ) ) {
            createInputsForSiblings[ entry[ 0 ] ] = entry[ 1 ];
        }
    } else {
        newProperty = {};
    }
    return {
        ...
        createInputsForSiblings: createInputsForSiblings
    };
};
