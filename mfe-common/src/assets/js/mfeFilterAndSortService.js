...
function processContainsFilter( filter, modelObjects ) {
    ...
    return modelObjects.filter( ( modelObject ) => {
        if( modelObject.props[ filter.columnName ] && modelObject.props[ filter.columnName ].uiValues[ 0 ] ) {
            return modelObject.props[ filter.columnName ].uiValues.join().toLowerCase().indexOf( filterValue ) !== -1;
        }
    } );
}
...
function processStartsWithFilter( filter, modelObjects ) {
    ...
    return modelObjects.filter( ( modelObject ) => {
        if( modelObject.props[ filter.columnName ] && modelObject.props[ filter.columnName ].uiValues[ 0 ] ) {
            return modelObject.props[ filter.columnName ].uiValues.join().toLowerCase().indexOf( filterValue ) === 0;
        }
    } );
}
/**
...
function processEndsWithFilter( filter, modelObjects ) {
    ...
    return modelObjects.filter( ( modelObject ) => {
        if( modelObject.props[ filter.columnName ] && modelObject.props[ filter.columnName ].uiValues[ 0 ] ) {
            const propValInLower = modelObject.props[ filter.columnName ].uiValues.join().toLowerCase();
...
function processEqualsFilter( filter, modelObjects ) {
    ...
    return modelObjects.filter( ( modelObject ) => {
        if( modelObject.props[ filter.columnName ] && modelObject.props[ filter.columnName ].uiValues[ 0 ] ) {
            return modelObject.props[ filter.columnName ].uiValues.join().toLowerCase() === filterValue;
        }
    } );
}
/**
...
function filterDispatcher( filter, modelObjects ) {
    let valueType = '';
    let validObjects = modelObjects.filter( ( object ) => {
        const property = object.props[ filter.columnName ];
        return property && property.propertyDescriptor && property.propertyDescriptor.valueType;
    } );
    if( validObjects.length > 0 ) {
        valueType = vmoSvc.getClientPropertyType( validObjects[0].props[ filter.columnName ].propertyDescriptor.valueType );
        switch ( valueType ) {
            case INTEGER:
            case DOUBLE:
            case FLOAT:
            case SHORT:
                validObjects = numericFiltersManager( filter, validObjects );
                break;
            case DATE:
                validObjects = dateFiltersManager( filter, validObjects );
                break;
            case STRING:
            case CHAR:
            case BOOLEAN:
        case OBJECT:
        case OBJECTARRAY:
                validObjects = textFiltersManager( filter, validObjects );
                break;
            default:
                break;
        }
...
export function sortModelObjectsByProp( modelObjects, sortByProp, ascending ) {
    if( Array.isArray( modelObjects ) && modelObjects.length > 1 && sortByProp ) {
        return modelObjects.sort( ( obj1, obj2 ) => {
            ...
            //For Value type OBJECT AND OBJECTARRAY considering property value as string
            if( valueType === OBJECT || valueType === OBJECTARRAY ) {
                val1 = obj1.props[ sortByProp ].uiValues.join();
                val2 = obj2.props[ sortByProp ].uiValues.join();
            }
            else if( valueType === INTEGER || valueType === DOUBLE || valueType === FLOAT || valueType === SHORT ) {
 ...
 * @return {modelObject[]} a sorted array of modelObjects
 */
export function sortModelObjectsBasedOnStringNumericValue( modelObjects, sortCriteria ) {
    let sortedModelObjects = modelObjects;
    if( Array.isArray( modelObjects ) && modelObjects.length > 1 && Array.isArray( sortCriteria ) && sortCriteria.length > 0 ) {
        const { fieldName, sortDirection } = sortCriteria[ 0 ];
        sortedModelObjects = sortModelObjectsByNumericProp( modelObjects, fieldName, sortDirection === 'ASC' );
    }
    return sortedModelObjects;
}
...
export default exports = {
    ...
    sortModelObjectsBasedOnStringNumericValue
};
