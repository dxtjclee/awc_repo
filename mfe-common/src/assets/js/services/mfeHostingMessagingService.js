...
let executeCommandFunctions = null;
...
function handleExecuteCommand( data ) {
    if( data ) {
        const func = executeCommandFunctions.get( data.key );
        if( func !== undefined ) {
            func( ...Object.values( data.value ) );
        } else {
 ...
 * @param {Function} executeFunction - the function to execute
 */
export function registerCommand( commandId, executeFunction ) {
    if( executeCommandFunctions === null ) {
        executeCommandFunctions = new Map();
    }
    executeCommandFunctions.set( commandId, executeFunction );
}
