 ...
 */
const initializeHosting = ( location, prefix,  iframeId ) => {
    ...
    return initSrc( location, prefix );
};
...
const initSrc = ( location, prefix ) => {
    ...
    for( const entry of queryParams.entries() ) {
        paramString = paramString.concat( `&${entry[0]}=${entry[1]}` );
    }
 ...
 *
 *
 * @function Updates src of iframe
 * @param {String} location - AW location

 */
export const updateSrc = function( location ) {
    const prefix = window.location.href.substring( window.location.href.indexOf( '#/' ) + 2, window.location.href.indexOf( '?' ) );
    initSrc( location, prefix );
    return {
        src: _src
    };
};
 ...
 */
export const initHosting = function( location, prefix,  iframeId = 'hostedIframe' ) {
    _clipboardService = ClipboardService.instance;
    if ( !prefix ) {
        prefix = window.location.href.substring( window.location.href.indexOf( '#/' ) + 2, window.location.href.indexOf( '?' ) );
    }
    ...
    initializeHosting( location, prefix,  iframeId );
