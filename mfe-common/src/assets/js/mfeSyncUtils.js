...
/**
 * If there is an input object on the sync port and the object type is valid, return it.
 * Otherwise return empty string.
 *
 * @param {Object} portsInputObject input object from the sync port
 * @param {StringArray} validInputTypes types that are valid as input
 *
 * @returns {Object} the appropriate input object
 */
export function getValidInputType( portsInputObject, validInputTypes ) {
    let singleInputObj = portsInputObject;
    if( _.isArray( portsInputObject ) && portsInputObject.length === 1 ) {
        singleInputObj = portsInputObject[0];
    }
    if( singleInputObj && singleInputObj.uid ) {
        const validInputType = validInputTypes.find( validType => singleInputObj.modelType.typeHierarchyArray.includes( validType ) );
        if( validInputType ) {
            return singleInputObj;
        }
    }
    return '';
}
