...
export let loadStatAttributesColumn = function( index ) {
    ...
    var start = index * 8 + 1;
    var end = index < 4 ? start + 8 : start + 1;
...
function getRequestURL( data ) {
    ...
    var reportURL = browserUtils.getBaseURL();
    reportURL += 'tc/micro/dpvservice/gra/execute/';
    reportURL += repTemplateUid;
    ...
    if ( !data.qryTypeState.useOrigTempSettings ) {
        ...
        } else if ( queryType === '1' ) //Last Jobs based on time
        {
            ...
            reportURL += '"&d="' + cDays + '"&h="' + cHrs + '"&m="' + cMins + '"';
            ...
            reportURL += '"&fd="' + cfromDate + '"&ed="' + ctoDate;
...
export let updateQueryTypeOrigSettings = function( subPanelContext, origSettingsVal ) {
    ...
    if( origSettingsVal ) {
        ...
        stateData.lastJobsDays = '1';
        stateData.lastJobsHrs = '0';
        stateData.lastJobsMins = '0';
        stateData.fromDate = '';
        stateData.toDate = '';
...
export let updateChartOrigSettings = function( state, origSettingsVal ) {
    ...
    stateData.useOrigTempSettings = origSettingsVal;
    if( origSettingsVal ) {
        stateData.chartType = '2';
        stateData.xAxisLabels = '1';
        stateData.yAxisLabels = '0';
...
export let resetCommonParams = function( data, qryTypeState ) {
    ...
    var dateRange = exports.setDateRange( qryTypeState, data.fromDate, data.toDate );
    ...
    for( var idx = 0; idx < queryTypeList.dbValue.length; idx++ ) {
        queryTypeList.dbValue[idx].selected = queryTypeList.dbValue[idx].propInternalValue === qryTypeState.qryType;
        queryTypeList.dbValues[idx].selected = queryTypeList.dbValues[idx].propInternalValue === qryTypeState.qryType;
    }
...
export let resetChartSettingsData = function( data, chartSettingsState ) {
    ...
    for( var idx = 0; idx < basisList.dbValue.length; idx++ ) {
        basisList.dbValue[idx].selected = basisList.dbValue[idx].propInternalValue === chartSettingsState.basis;
        basisList.dbValues[idx].selected = basisList.dbValues[idx].propInternalValue === chartSettingsState.basis;
    }
...
export let setDateRange = function( qryTypeState, fromDate, toDate ) {
    var endDate = new Date();
    var startDate = new Date();
    startDate.setDate( startDate.getDate() - 1 );

    let qryTypeData = { ...qryTypeState.value };
    qryTypeData.fromDate = getUnixTime( startDate );
    qryTypeData.toDate = getUnixTime( endDate );
    qryTypeState.update( { ...qryTypeData } );
    ...
    var fromDateTemp = _.clone( fromDate );
    var toDateTemp = _.clone( toDate );

    fromDateTemp.dbValue = startDate;
    toDateTemp.dbValue = endDate;
