...
export let queryRulesets = function () {
    ...
    // Call SOA to get ruleset datasets
    soaService.postUnchecked('ProductionManagement-2011-06-MeasurementDataQuery', 'queryRuleSetDataSet').then(
        function (response) {
            if (response && response.dpvRuleSetDsSet && response.dpvRuleSetDsSet.length > 0) {
                totalFound = response.dpvRuleSetDsSet.length;
                var uidToLoad = [];
                var vmo = [];
                //ignore the first element as first element will always be 'Siemens rule set'
                for (let idx = 1; idx < response.dpvRuleSetDsSet.length; idx++) {
                    uidToLoad.push(response.dpvRuleSetDsSet[idx].uid);
                }
                dmSvc.loadObjects(uidToLoad).then(function () {
                    var modelObjects = cdm.getObjects(uidToLoad);
                    for (var nn = 0; nn < modelObjects.length; nn++) {
                        vmo.push(viewModelObjectSvc.createViewModelObject(modelObjects[nn]));
                    }
                    outputData.totalFound = totalFound;
                    outputData.rulesetList = vmo;
