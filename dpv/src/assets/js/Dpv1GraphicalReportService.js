...
export let resubmitReportJob = function( selectedJobs ) {
    var deferred = AwPromiseService.instance.defer();
    var reportsIds = [];
    _.forEach( selectedJobs, function( job ) {
        reportsIds.push( job.props.ReportId.displayValues[0] );
    } );

    var $http = AwHttpService.instance;
    var launchInfoInput = reportsIds;

    var targetURL = browserUtils.getBaseURL() + 'tc/micro/dpvservice/gra/operation/resubmit';
...
export let cancelReportJob = function( selectedJobs ) {
    var deferred = AwPromiseService.instance.defer();
    var reportsIds = [];
    _.forEach( selectedJobs, function( job ) {
        reportsIds.push( job.props.ReportId.displayValues[0] );
    ...
    var targetURL = browserUtils.getBaseURL() + 'tc/micro/dpvservice/gra/operation/cancel';
...
export let createChart = function( type, startDate, endDate ) {
    var deferred = AwPromiseService.instance.defer();
    var repNames = [];
    var repCounts = [];
    ...
    var startDateParam = AwFilterService.instance( 'date' )( new Date( startDate ), 'yyyyMMdd' );
    var endDateParam = AwFilterService.instance( 'date' )( new Date( endDate ), 'yyyyMMdd' );

    var dateParam = startDateParam + '/' + endDateParam;

    var chartName = '';
    var url = browserUtils.getBaseURL() + 'tc/micro/dpvservice/gra/operation/viewcount/top/20/' + dateParam;
    chartName = dpvTextBundle.mostAccessedRepChartTitle;
    if( type === 'leastAccessed' ) {
        url = browserUtils.getBaseURL() + 'tc/micro/dpvservice/gra/operation/viewcount/bottom/20/' + dateParam;
