...
export let getInputObjects = function( subPanelContext ) {
    ...
    } else if( subPanelContext && subPanelContext.selected && _.isArray( subPanelContext.selected ) ) {
        return subPanelContext.selected;
    } else if ( subPanelContext && subPanelContext.selected && !_.isArray( subPanelContext.selected ) ) {
        return [ subPanelContext.selected ];
    }
...
export let prepareColumnList = function( data, sharedData, searchState ) {
    ...
    } else if( !appCtxService.ctx.reqSummaryTable && searchState && searchState.searchResponseInfo && searchState.searchResponseInfo.columnConfig.columns &&
...
export let clearSharedData = function( sharedData ) {
    ...
    appCtxService.unRegisterCtx( 'excelTemplateForExport' );
};
