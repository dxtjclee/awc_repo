    ...
    if( editHandler ) {
        ...
        if( dataSource ) {
            ...
            _.forEach( allEditableProperties, function( vmProp ) {
                ...
                if( vmProp && ( vmProp.isAutoAssignable || _uwPropSrv.isModified( vmProp ) ||  vmProp.propertyName.endsWith( 'awp0ProcessTemplates' ) && propValues && propValues.length > 0 && propValues[0].length > 0  ) ) {
...
export let autoAssignFileInfo = function( creationType, editHandler ) {
    let panelContext = appCtxService.getCtx( 'panelContext' );
    let fileSelectionData = {};
    if( panelContext ) {
        if( panelContext.fileName ) {
            let viewModelProps = [ { propertyName: 'object_name', dbValue: panelContext.fileName } ];
            assignInitialValues( viewModelProps, creationType, editHandler );
        }
        if( panelContext.files ) {
            const sourceObjects = panelContext.files[0].sourceObjects;
