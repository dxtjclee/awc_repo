    ...
    if( xrtState ) {
        ...
        if( xrtData ) {
            ...
            if( !vmoIn ) {
                ...
                // This block is to update the vmoIn with default uid from xrtData data.
                if( !vmoIn ) {
                    vmoIn = {};
                    vmoIn.operationName = operationName;
                    const selectedUid = xrtUtilities.getSelectedUid( xrtData );
                    vmoIn = viewModelObjectService.createViewModelObject( selectedUid, operationName, owningObjUid, xrtData.data.objects[ selectedUid ][ 0 ] );
                }
            }
