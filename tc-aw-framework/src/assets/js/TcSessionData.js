...
function _logSessionData( appCtxSvc, soaResponse ) {
    if( soaResponse.analyticsData ) {
        ...
        if( soaResponse.analyticsData.isDataCollectionEnabled || preInitData.isNecessaryDataEnabled ) {
...
export let parseSessionInfo = function( soaResponse ) {
    ...
    if( appCtxSvc.ctx?.preferences?.TC_Analytics_PEP_Message &&
        appCtxSvc.ctx.preferences.TC_Analytics_PEP_Message[ 0 ] &&
        appCtxSvc.ctx.preferences.TC_Analytics_PEP_Message[ 0 ] === 'false' ) {
