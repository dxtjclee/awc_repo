...
let sessionWebSocket = function() {
    ...
    if( _sessionWS ) {
        ...
        // window.open w/ target _self, the websocket connection is closed causing auto logout
        const open = window.open;
        window.open = ( url, target, windowFeatures ) => {
            if( target === '_self' ) {
                // if page is reloaded (nav), this won't be seen
                setTimeout( sessionWebSocket, 1000 );
            }
