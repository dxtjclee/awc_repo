...
/**
 * @module js/Att1InlineAuthoringEditService
 */
...
let notifySaveStateChanged = function( stateName ) {
    if( self.dataSource ) {
        ...
        context.dataSource.name = 'uniformParamDataProvider';
    ...
    leavePlaceService.registerLeaveHandler( {
        okToLeave: function( targetNavDetails, oldState, newState ) {
            ...
            if( !stillInFeaturesPageTreeView ) {
                exports.leaveConfirmation( callBackObj );
...
let _displayNotificationMessage = function() {
    ...
    if( !self._deferredPopup && editInProgress() ) {
        self._deferredPopup = awPromiseService.instance.defer();
        let buttonArray = [];
        let localeTextBundle = localeSvc.getLoadedText( 'Att1AttrMappingMessages' );
