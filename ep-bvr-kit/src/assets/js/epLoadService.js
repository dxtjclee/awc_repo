...
const CLOSE_WINDOWS = 'ep.closeOldWindows';
 ...
 * @param {String} errorMessage message to show when there are partial errors
 * @return {Promise} promise of the response
 */
export function loadObject( loadTypeInputs, replaceContext, errorMessage ) {
    const jsonInput = epLoadInputHelper.getLoadInputJSON( loadTypeInputs );
    let closeWindowOriginal = appCtxSvc.getCtx( CLOSE_WINDOWS );// remember the value for the fallback
    appCtxSvc.updatePartialCtx( CLOSE_WINDOWS, false );
    ...
    return promise.then( function( responseData ) {
        if( responseData.ServiceData && responseData.ServiceData.partialErrors ) {
            appCtxSvc.updatePartialCtx( CLOSE_WINDOWS, closeWindowOriginal );
            let err = errorMessage;
            if( err === undefined ) {
                err = messagingService.getSOAErrorMessage( responseData.ServiceData );
            }
...
/**
 * Load ACE objects - for AW components
 *
 * @param {*} inputObjects input as BOM line(s)
 * @returns {Array} of ACE objects
 */
export function loadAceObjects( inputObjects ) {
    if( !Array.isArray( inputObjects ) ) {
        inputObjects = [ inputObjects ];
    }
    const inputUids = _.map( inputObjects, 'uid' );
    const loadTypeInputs = epLoadInputHelper.getLoadTypeInputs( [ epLoadConstants.GET_AWB_ELEMENTS ], inputUids );
    return loadObject( loadTypeInputs, false ).then( response => {
        if( response.relatedObjectsMap ) {
            return _.map( inputUids, uid =>
                cdm.getObject( response.relatedObjectsMap[ uid ].additionalPropertiesMap2.mbc0awbElement[ 0 ] ) );
        }
        return [];
    } );
}
 ...
 */
function processPartialErrorsFromSOA( serviceData ) {
    let err = messagingService.getSOAErrorMessage( serviceData );
    messagingService.showError( err );
}
