...
function createAlternative( data ) {
    ...
    let openOnActionBox = data.openAlternativeOnActionBox.dbValue;
...
function isCloneAlternative() {
    let modelObjs = appCtxService.getCtx( EP_PAGE_CONTEXT );
    if( modelObjs.collaborationContext.props[ _epBvrConstants.MBC_MASTER_CC ].dbValues[ 0 ] ) {
        return true;
    }
    return false;
}
...
function createAlternativeWorkPackage( data, openOnActionBox ) {
    ...
    epSaveService.saveChanges( saveInputWriter, false, [ object ] ).then( function( result ) {
        ...
        if( openOnActionBox ) {
            ...
            messagingService.showInfo(  data.i18n.alternativeCreated.replace( '{0}', `"${data.packageName.dbValue}"` ) );
        }
    } );
}
