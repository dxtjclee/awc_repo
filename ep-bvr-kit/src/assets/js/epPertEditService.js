...
function createPublishPertData( eventData, graphModel, subPanelContext ) {
    ...
    _.forEach( eventData.saveEvents, function( event ) {
        if( event.eventType === 'create' ) {
            ...
            newlyAddedNodes.push( event.eventObjectUid );
        }
    } );
    ...
    if( objectUids && objectUids.length > 0 && newlyAddedNodes.length > 0 ) {
        const pertData = {
            nodes: [],
            edges: [],
            ports: []
        ...
        const modelObjsCreated = _.map( objectUids, ( uid ) => {
            const vmo = mfeViewModelObjectLifeCycleService.createViewModelObjectFromModelObject( cdm.getObject( uid ) );
 ...
 */
export function selectRecentPertNode( addedNodes, graphModel, newlyAddedNodeUids ) {
    //Fetch newly added nodes from save events
    let newlyAddedNodes = [];
    newlyAddedNodes = newlyAddedNodeUids && newlyAddedNodeUids.map( nodeUid => {
        if ( graphModel.dataModel.nodeModels.hasOwnProperty( nodeUid ) ) {
            return graphModel.dataModel.nodeModels[ nodeUid ].graphItem;
        }
        return;
    } );
    //if newly added nodes are not present and only one node is present in graph, add it in newlyAddedNodes to select it in graph
    if ( !newlyAddedNodes || newlyAddedNodes.length === 0 ) {
        const graphNodes = _.uniq( addedNodes.nodes );
        if ( graphNodes && graphNodes.length === 1 ) {
            newlyAddedNodes = graphNodes;
        }
    }
    if( newlyAddedNodes.length > 0 ) {
        ...
        graphModel.graphControl.setSelected( newlyAddedNodes, true, null );
    }
}
            ...
            epSaveService.saveChanges( saveInputWriter, true ).then( ( response ) => {
                if( !response.ServiceData.partialErrors ) {
                    if( response.saveEvents ) {
                        ...
                        if( relevantEvents ) {
                            const objUidToRemoveList = [];
                            relevantEvents.forEach( event => {
                                if( event.eventObjectUid === connectToObject.uid ) {
                                    const relatedEvents = event.relatedEvents;
                                    objUidToRemoveList.push( ...relatedEvents[ epSaveConstants.REMOVED_FROM_RELATION ] );
                                }
                            } );
