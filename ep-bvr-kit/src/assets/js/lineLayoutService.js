 ...
 * @return {Object} all the needed loaded data
 */
        ...
        return epLoadService.loadObject( loadtypeInput, false ).then(
            function( response ) {
                if ( response.ServiceData ) {
                    ...
                    lineModelObject = response.ServiceData.modelObjects[currentObjectUid];
...
export function setViewerDataInCtx( viewerData ) {
    const viewerRef = document.querySelector( '.aw-lineLayout-imageViewer' );
    viewerData.viewerRef = {
        current: viewerRef
    };
...
export function getSavedDatasetObject( saveResults, id ) {
    ...
    const savedObj = saveResults.find( result => result.clientID === id );
    const dataTypeObject = savedObj ? savedObj.saveResultObject : null;
...
/**
 * Get the line model object ( epPageContext.loadedObject )
 *
 * @returns {Object} lineModelObject the line model object
 */
export function getLineModelObject() {
