...
export const getSessionSection = function( isPerformCheck, tracking_cn, impacting_cn ) {
    const dataEntries = [];
    ...
    if( tracking_cn && rootLineUid !== '' ) {
        dataEntries.push( createTrackingCNEntry( tracking_cn ) );
    }

    if( impacting_cn && rootLineUid !== '' ) {
        dataEntries.push( createImpactingCNEntry( impacting_cn ) );
    }
...
const createTrackingCNEntry = function( tracking_cn ) {
...
const createImpactingCNEntry = function( impacting_cn ) {
    const rootL = epContextService.getPageContext().loadedObject ? epContextService.getPageContext().loadedObject.uid : '';
    if( rootL ) {
        return {
            entry: {
