...
function instantiateActivity( libraryActivity, inputObject, selectedObjects, properties ) {
    ...
    //Do not re-load Accountablity after activity creation
    const reloadInput  = epReloadService.unregisterReloadInput( _epLoadConstants.EP_ASSIGNMENT_INDICATION );
    return epSaveService.saveChanges( saveWriter, true, relatedObjects ).then( function( saveResponse ) {
        if ( saveResponse ) {
            ...
            epReloadService.registerReloadInput( _epLoadConstants.EP_ASSIGNMENT_INDICATION, reloadInput );
        }
    }, function( error ) {
        epReloadService.registerReloadInput( _epLoadConstants.EP_ASSIGNMENT_INDICATION, reloadInput );
    } );
