...
export function rendererForExternalFlowIndication( vmo, containerElement ) {
    ...
    if( graph && graph.graphModel ) {
        ...
        selectedPertNodes = [ epObjectPropertyCacheService.getProperty( vmo.uid, 'inputObjectForSelectedVmo' ) ];
    }
 ...
 *
 * @param {Array} inputObject - the inputObject
 * @param {Object} vmos - the vmo for the cell
 * @param {string} columnsToUpdate - the column name
 */
function updateInputObjectToVmo( inputObject, vmos ) {
    if ( !inputObject && !vmos ) {
        return;
    }
    if ( inputObject && inputObject.uid ) {
        vmos.forEach( ( vmo ) => {
            epObjectPropertyCacheService.setProperty( vmo.uid, 'inputObjectForSelectedVmo', inputObject );
        } );
    }
}
