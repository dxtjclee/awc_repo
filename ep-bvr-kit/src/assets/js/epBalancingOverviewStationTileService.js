...
export function epBalancingOverviewStationTileRender( props ) {
    return (
        <div className='aw-epBalancing-processResourceBar' onClick={() => stationClicked( props )} onKeyDown={() => stationClicked( props )} role='button' tabIndex='0'
            title={ props.vmo.props.bl_rev_object_name.dbValues[0] + ': ' + epBalancingLabelsService.stringToFloat( props.vmo.props.elb0cycleTime.dbValue ) }>
            {renderTimeBar( props.vmo.props, props.balancingData.loadedData.maxTimeInLine )}
        </div>
    );
}
 ...
 */
function stationClicked( props ) {
    epBalancingSelectionService.updateSelection( props.balancingData, props.vmo );
}
