...
/**
 * remove EBOM revision in work Package
 * @param {Object} selectedObject - selected Object from search popup
 */
export function replaceEBOMRevisionInCC( selectedEBOMObjectFromTile, selectedEBOMObjectFromPopup, revisionRule ) {
    const saveInputWriter = saveInputWriterService.get();
    const epTaskPageContext = appCtxService.getCtx( 'epTaskPageContext' );
    const ccObj = epTaskPageContext.collaborationContext;
    const objectToRemove =  selectedEBOMObjectFromTile.uid;
    const objectToAdd =  selectedEBOMObjectFromPopup.uid;
    const revRule = revisionRule.dbValue;
    const ccObject = {
        id: [ ccObj.uid ]
    };
    const objectToReplaceInCC = {
        Replace: [ objectToRemove, objectToAdd ],
        RevisionRule: [ revRule ]
    };
    saveInputWriter.replaceObjectInCC( ccObject, objectToReplaceInCC );
    const objectToLoad = [ ccObj.uid, objectToAdd, revRule ];

    return dataManagementService.loadObjects( objectToLoad ).then( function() {
        const ccObject = cdmSvc.getObject( ccObj.uid );
        const objectToAddObj = cdmSvc.getObject( objectToAdd );
        const revRuleObj = cdmSvc.getObject( revRule );
        const objectToRemoveObj = cdmSvc.getObject( objectToRemove );
        const relatedObjects = [ ccObject, objectToAddObj, objectToRemoveObj, revRuleObj ];
