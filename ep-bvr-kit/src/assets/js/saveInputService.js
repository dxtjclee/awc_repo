...
export default class SaveInput {
    ...
    addSessionInformation( performCheck, tracking_cn, impacting_cn ) {
        this.sections[ epSaveConstants.SESSION ] = epSessionService.getSessionSection( performCheck, tracking_cn, impacting_cn );
    }
    ...
    addReviseInput( reviseObjects, action = epReviseHelper.CONSTANTS.EDIT_PROPERTIES ) {
        if( reviseObjects ) {
            const reviseObjectsArray = _.isArray( reviseObjects ) ? reviseObjects : [ reviseObjects ];
            const reviseObjectsUids = _.map( reviseObjectsArray, object => object.uid );
            const entry = {
                Object: prepareNameToValuesMap( {
                    id: reviseObjectsUids
                } ),
                Action: prepareNameToValuesMap( {
                    name: action
                } )
            };
    ...
    addAssignedObject( object, operationObject ) {
        const entry = {};
        ...
        if ( operationObject ) {
            entry.AssignedOperations = prepareNameToValuesMap( operationObject );
        }
        this.addEntryToSection( epSaveConstants.OBJECTS_TO_MODIFY, entry );
    }
    ...
    addModifiedPropertyforObjects( objectIds, propertyName, propertyValues ) {
        const propData = prepareNameToValuesMap( {} );
        propData.nameToValuesMap[propertyName] = propertyValues ? convertToStringIfNeeded( propertyValues ) : '';
        const entryarray = [];
        objectIds.forEach( ( val )=>{
            let obj = {
                entry:{
                    Object: prepareNameToValuesMap( {
                        id: val
        ...
        let section = this.sections[epSaveConstants.OBJECTS_TO_MODIFY];
        if ( !section ) {
            section = {
                sectionName: epSaveConstants.OBJECTS_TO_MODIFY,
                dataEntries: entryarray
            };
            this.sections[epSaveConstants.OBJECTS_TO_MODIFY] = section;
        }
    }
    ...
    /**
     * @param {Object} object
     * @param {AllocatedOperations} operationsAllocated
     */

    addAllocatedOperations( object, operationsAllocated ) {
        let entry = {};
        if( object ) {
            entry.Object = prepareNameToValuesMap( object );
        }
        if( operationsAllocated ) {
            entry.AllocatedOperations = prepareNameToValuesMap( operationsAllocated );
        }
    ...
    /**
     * @param {Object} object
     * @param {Object} objectToReplace input for ConnectedStructures
     */
    replaceObjectInCC( object, objectToReplace ) {
        let entry = {};
        if( object ) {
            entry.Object = prepareNameToValuesMap( object );
        }
        if( objectToReplace ) {
            entry.ConnectedStructures = prepareNameToValuesMap( objectToReplace );
        }
        this.addEntryToSection( epSaveConstants.OBJECTS_TO_MODIFY, entry );
    }
    ...
    /**
     * @param { Object } inputObjUid inputObjUid

     */
    createDIPA( inputObjUid ) {
        const entry = {};
        entry.Object = prepareNameToValuesMap( { id: [ inputObjUid ] } );
        entry.DIPA = prepareNameToValuesMap( {
            Create: inputObjUid,
            isRecursive: 'false'
        } );
        this.addEntryToSection( epSaveConstants.OBJECTS_TO_MODIFY, entry );
    ...
    /**
     * @param { Object } inputObjUid inputObjUid
     * @param { array[] } impactedUids impactedUids
     */

    clearImpactIndications( inputObjUid, impactedUids ) {
        const entry = {};

        entry.Object = prepareNameToValuesMap( { id: [ inputObjUid ] } );

        entry.ImpactIndication = prepareNameToValuesMap( { CLEAR: impactedUids } );
        this.addEntryToSection( epSaveConstants.OBJECTS_TO_MODIFY, entry );
    }

    /**
     *  @param { array[] } impactHandledUids impactHandledUids
     */
    restoreImpactIndications( impactHandledUids ) {
        const entry = {};
        entry.ImpactIndication = prepareNameToValuesMap( { RESTORE: impactHandledUids } );
        this.addEntryToSection( epSaveConstants.OBJECTS_TO_MODIFY, entry );
    }

    addPropagateProperties( source, target, properties ) {
        var entry = {};
        if( source ) {
            entry.Object = prepareNameToValuesMap( {
                id: source.uid
        ...
        if( target ) {
            entry.PropagateProperties = prepareNameToValuesMap( {
                targetObjects: target.uid,
                properties
            } );
        }
        this.addEntryToSection( epSaveConstants.OBJECTS_TO_MODIFY, entry );
