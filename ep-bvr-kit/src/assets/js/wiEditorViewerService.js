...
const WI_DATA_LIST_EVENT_TYPE = 'WIDataList';
 ...
 */
function onInit( isIndicationOn, occContext ) {
    ...
    if( loadedProductViewModel && loadedProductViewModel.uid ) {
        ...
        if( assemblyStructure && assemblyStructure.props && assemblyStructure.props.bl_child_lines !== undefined ) {
            assemblyStructure.props.bl_child_lines.dbValues.map( uid =>
    ...
    if( selectedObj ) {
        ...
        return epLoadService.loadObject( processloadInputs, false ).then( function( output ) {
            ...
            editorsToLoad.editorsData = wiObjectUIDs.map( objUID => {
                    return updateEditor( objUID, output.relatedObjectsMap[ objUID ], output.ServiceData.modelObjects[ objUID ], isIndicationOn, occContext  );
            } );
 ...
 */
function updateEditor( objUID, stepData, stepObject, indicationOn, occContext ) {
    ...
    if(occContext){
        occContext.hideCreateAndAddMoreButton = true;
    } 
...
export function destroy() {
    ...
    window.removeEventListener( 'beforeunload', wiEditorService.onBeforeUnload );
}
/**
...
export function handleSaveCreateEvent( eventData, saveCreateEvents, dataProvider ) {
    ...
    if ( createdObjs.length > 0 ) {
        ...
        const WIDataListEvent = eventData.saveEvents.filter( event => event.eventType === WI_DATA_LIST_EVENT_TYPE );
        const refIndex = getRefIndexOfNewObj( WIDataListEvent[0], createdObjs[ 0 ] );
        loadedVMObjects.splice( refIndex, 0, ...createdObjs );
        //validate index of editor, refresh in case index is incorrect
        if( loadedVMObjects[createdObjs[ 0 ].uid] !== WIDataListEvent[0].eventData.indexOf( createdObjs[ 0 ].uid ) ) {
            loadedVMObjects.sort( ( obj1, obj2 ) => WIDataListEvent[0].eventData.indexOf( obj1.uid ) - WIDataListEvent[0].eventData.indexOf( obj2.uid ) );
        }
...
/**
 *
 * @param {Objecy} WIDataListEvent WI data list event
 * @param {Object} createdObj new created object
 * @returns {Int} ref index
 */
function getRefIndexOfNewObj( WIDataListEvent, createdObj ) {
    return WIDataListEvent.eventData ? WIDataListEvent.eventData.findIndex( ( objUid ) => objUid === createdObj.uid ) : 0;
}
/**
