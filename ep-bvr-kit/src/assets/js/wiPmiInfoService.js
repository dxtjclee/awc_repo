...
function getPMIDetails( vmo, currentScopeUid, currentAssyUid ) {
    let characteristicUid = '';
    if( mfeTypeUtils.isOfType( vmo, wiPMIConstants.MCI_INSPECTION_REVISION ) ) {
        if( vmo.props[ wiPMIConstants.MCI_CHARACTERISTIC_OF_INSPECTION ] ) {
            characteristicUid = vmo.props[ wiPMIConstants.MCI_CHARACTERISTIC_OF_INSPECTION ].dbValues[ 0 ];
        } else {
...
function loadPmiDetails( characteristicUid, currentScopeUid, currentAssyUid ) {
    if( characteristicUid ) {
        ...
        return epLoadService.loadObject( loadInputTypes, false ).then( () => {
            ...
            const pmiTextArray = epPropCacheSvc.getProperty( characteristicUid, wiPMIConstants.PMI_TEXT );
            const pmiText = Array.isArray( pmiTextArray ) ? pmiTextArray.join( '\n' ) : '';
