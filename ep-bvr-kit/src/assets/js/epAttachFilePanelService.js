...
function getFileName( addPanelState ) {
    ...
    if ( addPanelState.references.length > 0 ) {
        ...
        if( addPanelState.fileExtension ) {
            referenceExtension = addPanelState.fileExtension;
        }
...
/**
 * getRelationMap
 * @param {Array} allowedRelations preference values - allowed relations per type
 * @returns {Object} map of type to all allowed relations
 */
export function getRelationMap( allowedRelations ) {
    if ( !allowedRelations || _.isEmpty( allowedRelations ) ) {
        return {};
    }

    return _.reduce( allowedRelations, ( relationMap, allowedRelation ) => {
        const [ relation, type ] = allowedRelation.split( '.' );
        ( relationMap[type] || ( relationMap[type] = [] ) ).push( relation );
        return relationMap;
    }, {} );
}
