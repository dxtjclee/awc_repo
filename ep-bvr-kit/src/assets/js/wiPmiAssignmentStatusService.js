 ...
 * @returns {Promise<boolean>} ture/false
 */
function hasMismatchedPmi( objUid ) {
    if( objUid ) {
        ...
        if( pageContext && pageContext.loadedObject ) {
            ...
            if( assignedPmiPropObj === undefined ) {
                return dmsSvc.getProperties( [ objUid ], [ 'epw0Inspections' ] ).then( () => {
                    return isAnyImpactedPmi( cdm.getObject( objUid ).props.epw0Inspections.dbValues );
                } );
            } else if( assignedPmiPropObj && assignedPmiPropObj.dbValues.length > 0 ) {
                return AwPromiseService.instance.resolve( isAnyImpactedPmi( assignedPmiPropObj.dbValues ) );
            }
        }
    }
...
/**
 *
 * @param {Array} pmiList pmi uid list
 * @returns {boolean} ture/false
 */
function isAnyImpactedPmi( pmiList ) {
    if( !Array.isArray( pmiList ) ) {
        return false;
    }
    for( const pmiUid of pmiList ) {
...
function syncToPmiTable( targetPmisDataProvider, selectedPmis ) {
    ...
    const selectedMetaObjects = selectedPmis[0].type === epBvrConstants.MCI_PMI_META_DATA ? selectedPmis.map( meta => meta.uid ) : selectedPmis.map( chx => chx.props.mci0pmiMetaData.dbValues[0] );
