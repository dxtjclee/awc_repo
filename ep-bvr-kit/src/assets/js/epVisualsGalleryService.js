    ...
    if( mfeTypeUtils.isOfTypes( object, [ epBvrConstants.SNAPSHOT_OBJECT_TYPE, epBvrConstants.WEB_LINK_OBJECT_TYPE, epBvrConstants.FULL_TEXT_OBJECT_TYPE, epBvrConstants.CME_REPORT_OBJECT_TYPE ] ) ) {
        ...
        if( displayName === null ) {
            if( props.object_name ) {
                ...
                displayName = props.original_file_name.uiValues[ 0 ];
            }
        }
    }
 ...
 */
export function sortDatasets( datasetsToShow ) {
...
export function updateGalleryPanel( selectedObjId ) {
    ...
    if( selectedObjId ) {
        ...
        return epLoadService.loadObject( loadTypeInputs, false ).then( function( output ) {
            ...
            let datatsetaToShowWithUrl = [];
            datasetsToShow.forEach( dataset => {
                const fileTicket = output.relatedObjectsMap[dataset.uid].additionalPropertiesMap2.Tickets[0];
                dataset.props.imageUrl = createImageUrlProperty( fileTicket, dataset );
                datatsetaToShowWithUrl.push( dataset );
            } );
            datasetsToShow = sortDatasets( datatsetaToShowWithUrl );
...
/**
 * creates a image url property
 *
 * @param {String} ticket - file ticket
 * @param {object} dataset - dataset
 * @rerurn {object}property
 */
function createImageUrlProperty( ticket, dataset ) {
    ...
    return viewModelObjectSvc.constructViewModelProperty( customProperty, 'imageUrl', dataset );
}
 ...
 *
 * @param {String} ticket - file ticket
 * @rerurn {String} image url
 */
function getImageUrl( ticket ) {
    return browserUtils.getBaseURL() + fmsUtils.getFMSUrl() + fmsUtils.getFilenameFromTicket( ticket ) + '?ticket=' + ticket;
}
...
function onBeforeUnload( e ) {
    if( appCtxService.getCtx( 'visualsGallery.editInProgress' ) ) {
        // Cancel the event
        e.preventDefault();
        e.returnValue = '';
    }
