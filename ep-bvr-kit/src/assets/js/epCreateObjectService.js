...
function filterModelTypes( typeListInfo, subTypeExclusionList ) {
    ...
    if( subTypeExclusionList && subTypeExclusionList.length > 0 && typeListInfo && typeListInfo.length > 0 ) {
        //find the type to be excluded in the heirarachy of Types to be created, if found object should be filterd out.
        filteredTypeList = typeListInfo.filter( type => {
            const intersection = _.intersection( subTypeExclusionList, type.typeHierarchyArray );
...
function registerPolicy() {
    const createObjectPolicy = {
        ...
        {
            name: epBvrConstants.MFG_BVR_WORKAREA,
            properties: [ {
                name: epBvrConstants.ME_LINKED_BOP
            } ]
        }
        ]
...
export function createObject( viewModelData, connectedTo, policyId, workPackage, newObjectId, predecessor, isResequenceNeeded, targetAssembly, creationType ) {
    ...
    if ( predecessor ) {
        ...
        if ( epAllowAutoRegenerateFN && epAllowAutoRegenerateFN[0] === 'true' ) {
            addRegenarateFindNumberInput( saveWriter, connectedTo );
        }
    }
    if( workPackage ) {
        const ccObject = {
    ...
    return epSaveService.saveChanges( saveWriter, true, relatedObjects ).then( function( serviceResponse ) {
        propPolicySvc.unregister( policyId.dbValue );
...
function addObjectsToResequence( before, parent, newObj, saveWriter ) {
    ...
    if( children && children.length > 0 ) {
        ...
        if ( index === children.length - 1 ) {
            ...
            newObjectFindNo = Math.round(( beforeObjectFindNo + afterObjectFindNo ) / 2);
        }
...
/**
 * registerReloadSectionForCreate
 */
export function registerReloadSectionForCreate( name, type, newObjectID, additionalLoadParams ) {
    if( !type ) { return; }
    ...
    // then it will be empty at this point. Lets fill it. & Don't modify orginal subPanelContext
    let reloadTypes = _.cloneDeep( type );
    reloadTypes = Array.isArray( reloadTypes ) ? reloadTypes : [ reloadTypes ];
    reloadTypes.forEach( ( reloadType ) => {
        if( !reloadType.object ) {
            reloadType.object = newObjectID;
            reloadType.additionalLoadParams = additionalLoadParams;
        }
    } );
    epReloadService.registerReloadInput( name, reloadTypes );
}
