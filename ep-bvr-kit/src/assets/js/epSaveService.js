...
export function saveChanges( saveInputWriter, performCheck, relatedObjects, returnEventsOnError = false, resourceBundle = 'EPServicesMessages' ) {
    ...
    saveInputWriter.addSessionInformation( performCheck, AwStateService.instance.params.tracking_cn, AwStateService.instance.params.impacting_cn );
    ...
    return soaService.postUnchecked( 'Internal-MfgBvrCore-2022-12-DataManagement', 'saveData4',
        ...
        processRelatedObjectsMapForTreeTable( result );
        ...
        if( saveEvents && saveEvents.length > 0 ) {
            ...
            const parsedAllocationIndicationEvents = parseAllocationIndicationSaveEventsData( saveEvents );
            ...
            if( parsedAllocationIndicationEvents && !_.every( parsedAllocationIndicationEvents, _.isEmpty ) ) {
                eventBus.publish( 'ep.allocationIndicationEvents', parsedAllocationIndicationEvents );
            }
            ...
            if( sequenceChanged( saveEvents ) ) {
                eventBus.publish( 'ep.sequenceChanged' );
            }
        }
        return result;
    }, function( error ) {
...
/**
 * Process RelatedObjectsMap For Tree Table
 *
 * @param {Object} response - the save response
 */
function processRelatedObjectsMapForTreeTable( response ) {
    const objUIDsToSelect = response.saveEvents ? response.saveEvents.filter( event =>
        event.eventType === epSaveConstants.ADDED_TO_RELATION || event.eventType === epSaveConstants.CREATE_EVENT || event.eventType === epSaveConstants.RESEQUENCE_EVENT )
        .flatMap( event => event.eventObjectUid ).filter( ( value, index, self ) => self.indexOf( value ) === index ) : []; //TODO TCM-419890 - Refactor the way of finding new nodes

    let relatedObjectMapKey = [ epBvrConstants.RELATED_OBJECT_CHILDREN_KEY, epBvrConstants.HAS_CHILDREN_PROP ];
    for( let objectUid in response.relatedObjectsMap ) {
        if( doesResponseContainRelatedObjectsMapKey( objectUid, response, relatedObjectMapKey ) ) {
            let hasChildrenValue = response.relatedObjectsMap[ objectUid ].additionalPropertiesMap2[ epBvrConstants.HAS_CHILDREN_PROP ][ 0 ];
            epObjectPropertyCacheService.setProperty( objectUid, epBvrConstants.HAS_CHILDREN_PROP, [ hasChildrenValue ] );
            if( hasChildrenValue === 'TRUE' ) {
                let childAssemblyValue = response.relatedObjectsMap[ objectUid ].additionalPropertiesMap2[ epBvrConstants.RELATED_OBJECT_CHILDREN_KEY ];
                if( childAssemblyValue ) {
                    epObjectPropertyCacheService.updateAndNotifyPropertyChange( objectUid, epBvrConstants.RELATED_OBJECT_CHILDREN_KEY, childAssemblyValue, objUIDsToSelect );
                }
            } else {
                epObjectPropertyCacheService.removeAndNotifyPropertyChange( objectUid, epBvrConstants.RELATED_OBJECT_CHILDREN_KEY );
            }
        }
    }
}
        ...
        if( saveEvent.eventData ) {
            ...
            if( eventData && eventData[ 0 ] ) {
                ...
                _.forEach( eventData, data => {
                    if( !parsedSaveEvents[ data ] ) {
                        parsedSaveEvents[ data ] = [];
                    }
                    parsedSaveEvents[ data ].push( parsedMultipleEvent );
                } );
            }
        }
    }
...
/**
 * parseAllocationIndicationSaveEventsData
 * @param {*} eventDataArray all save events
 * @returns {Object} allocation indication events, parsed
 */
function parseAllocationIndicationSaveEventsData( eventDataArray ) {
    if( !eventDataArray ) {
        return null;
    }

    const accountabilityCheckEvents = _.filter( eventDataArray, ( eventData ) =>
        eventData.eventType === epSaveConstants.ACCOUNTABILITYCHECK_EVENT && eventData.eventData );

    const missingInSource = _.filter( accountabilityCheckEvents, ( eventData ) =>
        parseInt( eventData.eventData[ 0 ] ) === -1 );
    const allocated = _.filter( accountabilityCheckEvents, ( eventData ) =>
        parseInt( eventData.eventData[ 0 ] ) > -1 && parseInt( eventData.eventData[ 0 ] ) < 4 );
    const notAllocated = _.filter( accountabilityCheckEvents, ( eventData ) =>
        parseInt( eventData.eventData[ 0 ] ) === 4 );
...
const sequenceChanged = ( saveEvents ) => {
    return _.find( saveEvents, event => event.eventType === epSaveConstants.RESEQUENCE_EVENT ) !== undefined;
};
