...
export function destroy() {
    let forceReload = appCtxService.getCtx( 'forceReload' );
    // Don't destroy if never been initialized
    if( !isInitialized ) {
        return;
    }
    appCtxService.unRegisterCtx( 'forceReload' );
    eventBus.publish( 'cdm.cleanCache', {} );
    let currentStateName = AwStateService.instance.current.name;
    let currentStateParent = AwStateService.instance.current.parent;
    // Don't destroy if EP State
    if( !forceReload && ( EP_STATES.includes( currentStateName ) || EP_STATES.includes( currentStateParent ) ) ) {
        return;
    }
    ...
    appCtxService.updatePartialCtx( 'objectQuotaContext.useObjectQuota', originalUseObjectQuotaValue && !forceReload );
