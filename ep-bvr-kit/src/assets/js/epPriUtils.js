 ...
 * @returns {*} chart data
 */
export function prepareContainers( line, station, processResources, selectedProcessResource ) {
    const containers = [];
    const flows = [];
    _.forEach( processResources, pr => {
        if( pr.props ) {
            let allocateByPv = undefined;
            if ( pr.props && pr.props.elb0allocatedOpsByPV ) {
            ...
            const capacity = ( pr.props && pr.props.capacity.dbValues[0] ) | 0; // convert to int
            const taktTime = parseFloat( pr.props && pr.props.elb0taktTime.dbValues[0]  ?  pr.props && pr.props.elb0taktTime.dbValues[0]  : 0 );

            const container = {
                id: pr.uid,
                name: prName,
                time: parseFloat( pr.props && pr.props.elb0workContentByPV.dbValues[0] ? pr.props.elb0workContentByPV.dbValues[0] : 0 ),
                ...
                isSelected: pr.uid === selectedProcessResource
            };
    ...
    return {
        taktTime: parseFloat( station && station.props && station.props.elb0taktTime.uiValues[0] ? station.props.elb0taktTime.uiValues[0] : 0 ),
        containers: containers,
        arrows: flows,
        cycleTime: parseFloat( station && station.props && station.props.elb0cycleTime.uiValues[0] ? station.props.elb0cycleTime.uiValues[0] : 0 ),
        lineTaktTime: line.props.elb0taktTimeConverted && line.props.elb0taktTimeConverted.displayValues[0] ? parseFloat( line.props.elb0taktTimeConverted.displayValues[ 0 ] ) : 0
...
const initializeItems = function( container, scalingRatio, taktTime, yPos, itemsMap ) {
    _.forEach( container.items, function( item ) {
        ...
        if ( item.hasOwnProperty( 'hasWaitTimeAfter' ) && !item.hasWaitTimeAfter && item.width > UI_CONSTANTS.ITEMS_SPACING ) {
 ...
 * @returns {ArrowHelper} helper
 */
export function getArrowHelper( data, arrowData ) {
    return new ArrowHelper( data, arrowData );
}

const ArrowHelper = function( data, arrowData ) {
    this.data = data;
...
ArrowHelper.prototype.startArrow = function( e, fromItem, fromPort ) {
    let target = e.target;
    if ( fromPort ) {
        const id = $( target ).attr( 'port-id' );
        target = $( `rect[item-id="${id}"]` ).get( 0 );
    }
    this.mouseDeltaX = target.getBoundingClientRect().left - fromItem.x;
    this.mouseDeltaY = target.getBoundingClientRect().top - fromItem.y;
    ...
    for ( const containerIndex in this.data.containers ) {
        for ( const itemIndex in this.data.containers[containerIndex].items ) {
            ...
            if ( this.current.fromItem !== item && !areItemsFromSameContainer( this.current.fromItem, item ) &&
                ...
                this.arrowData.inportItem = item.id;
                this.arrowData.update( this.arrowData );
...
MoveHelper.prototype.moveItem = function( e ) {
    ...
    for ( let index = 0; index < container.items.length - 1; index++ ) {
        ...
        if ( this.originalItem !== leftItem && this.originalItem !== rightItem
            && leftItem.x + leftItem.width / 2 + 1 < mouseX && mouseX < rightItem.x + rightItem.width / 2 - 1 ) {
            this.isDroppingToNewPlace = true;
...
const addFlow = function( flowObj, arrowData ) {
    const predecessorObj = {
        predecessor: flowObj.fromItem.id,
        object: flowObj.toItem.id
    };

    addPredecessor( predecessorObj, arrowData );
};

const addPredecessor = function( predecessorInfo, arrowData ) {
    const saveInputWriter = saveInputWriterService.get();
    const predecessorInfoObjectForSOA = {
        objectId: predecessorInfo.object,
        predecessorId: predecessorInfo.predecessor
    };

    let relatedObjects = [ cdm.getObject( predecessorInfo.object ) ];
    if ( predecessorInfo.predecessor ) {
        relatedObjects.push( cdm.getObject( predecessorInfo.predecessor ) );
    } else {
        predecessorInfoObjectForSOA.predecessorId = '';
    }

    saveInputWriter.addPredecessor( predecessorInfoObjectForSOA, 'true' );
    arrowData.showUpdateMessage = true;
    arrowData.inportItem = '';
    arrowData.outportItem = '';
    arrowData.update( arrowData );
    epSaveService.saveChanges( saveInputWriter, true, relatedObjects );
};

const deleteFlow = function(  flowObject, arrowData ) {
    const { deleteFlowConfirmation, remove, cancel } = localeService.getLoadedText( 'PriMessages' );
    mfgNotificationUtils.displayConfirmationMessage( deleteFlowConfirmation, remove, cancel ).then( () => {
        const saveInputWriter = saveInputWriterService.get();
        saveInputWriter.deleteFlow( flowObject, 'true' );
        const relatedObjects = [ cdm.getObject( flowObject.fromId ),
            cdm.getObject( flowObject.toId ) ];
        arrowData.showUpdateMessage = true;
        arrowData.update( arrowData );
        return AwPromiseService.instance.resolve( epSaveService.saveChanges( saveInputWriter, true, relatedObjects ) );
    } );
};
