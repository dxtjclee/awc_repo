    ...
    _defineConverters() {
        ...
        conversion.for( 'upcast' ).elementToElement( {
            ...
            model: ( viewElement, { writer: modelWriter } ) => {
                ...
                const occ = viewElement._attrs.get( 'occ' );

                if( occ ) {
                    return modelWriter.createElement( STANDARD_TEXT_ELEMENT, { uid, name, occ } );
                }
        ...
        conversion.for( 'upcast' ).elementToElement( {
            ...
            model: ( viewElement, { writer: modelWriter } ) => {
                if(viewElement.childCount > 0 && viewElement.getChild( 0 ) && viewElement.getChild( 0 ).data ){
                    const content = viewElement.getChild( 0 ).data ;

                    return modelWriter.createElement( DIVISION_ELEMENT, { content, bold: true } );
                }
                return modelWriter.createElement( DIVISION_ELEMENT);
            }
        } );
        ...
        function createStxPlaceholderView( modelItem, viewWriter ) {
            ...
            const occ = modelItem.getAttribute( 'occ' );
            if( occ ) {
                attributes.occ = occ;
            }

            return viewWriter.createContainerElement( STANDARD_TEXT_ELEMENT, attributes );
        }
        /**
         *
         * @param {Object} modelItem model item
