...
export let createTranslationDelivery = function( data ) {
    ...
    if( deliverDecomposedTopic === true ) {
        ...
        if( ( data.deliverTopicsOutForTransl.dbValue === true || data.deliverTopicsOutForTransl.displayValues[ 0 ].toUpperCase() === 'TRUE' ) && deliverOutOfSync ) {
...
export let updateDeliverDecomposed = function( data ) {
    ...
    return { deliverOutOfSync: { isEditable: isEditable, isEnabled: isEditable }, deliverTopicsOutForTransl: { isEditable: isEditable, isEnabled: isEditable } };
};
