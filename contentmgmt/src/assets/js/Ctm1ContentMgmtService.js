...
export let copyGraphicReference = function( data, selected ) {
    let systemIdValue = '';
    if( selected.props.systemId ) {
        systemIdValue = selected.props.systemId.dbValue;
    }

    // text area method
    let textArea = document.createElement( 'textarea' );

    if( data.dataset.props.ref_list !== null && data.dataset.props.ref_list.dbValues.length > 0 ) {
        textArea.value = '<image href="' + systemIdValue + '"/>';
    }
