...
export default class StructureViewerService extends AwBaseService {
            ...
            if( this._lastViewerCtxObj && !this._lastViewerCtxObj.isConnectionClosed() &&
                ...
                if( Array.isArray( productCtxInfo.props.awb0ContextObject.dbValues ) &&
                    ...
                    this._lastViewerCtxObj.getSessionMgr().updateAppSession(
                        ...
                        this._lastProductContextInfoObj = null; //to reload where session saved from other tabs
                    },
                    errorMsg => {
                        this._handleTcVisSessionUpdateError( errorMsg );
                    } );
            ...
            self._productContextInfoChangedEvent = eventBus.subscribe( 'productContextChangedEvent', () => {
                ...
                self._getLastModDate( self._getOccmgmtContextKey() ).then( ( LMD ) => {
                    if( self.createVisSessionifAppSessionBeingOpened( productCtxInfo, LMD ) ) {
                        ...
                        self._lastProductContextInfoObj = self.getPCIModelObject( self._getOccmgmtContext() );
                    }
                } );
    ...
    _isDisableBookMarkApplicable() {
        ...
        let skipAutoBookmark = appCtxService.getCtx( 'skipAutoBookmark' );
        if( !_.isUndefined( skipAutoBookmark ) && !_.isNull( skipAutoBookmark ) ) {
            return skipAutoBookmark;
        }
     ...
     */
    isApplyDeltaUpdate( aceContext ) {
        let self = this;
        const returnPromise = AwPromiseService.instance.defer();
        //Delta update is not available if TC version below 14.3
        ...
        viewerPreferenceService.getUseBomDeltaStructureUpdate().then( function( isEnableDeltaUpdate ) {
            let applyDelta = false;
            if( isEnableDeltaUpdate === 'true' ) {
                ...
                if( openedObj.modelType.typeHierarchyArray.indexOf( 'ItemRevision' ) > -1 ) {
                    let newProductCtx = self.getPCIModelObject( aceContext );
            ...
            returnPromise.resolve( applyDelta );
        } ).catch( function( error ) {
            logger.error( 'Failed to validate preference VIS_USE_BOM_DELTA_STRUCTURE_UPDATE or opened object type : ' + error );
