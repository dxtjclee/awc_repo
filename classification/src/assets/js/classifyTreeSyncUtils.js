 ...
 * @param {Context} classifyState the classify state to clean.
 */
export let clearSelection = function( selectionModel, classifyState ) {
 ...
 * @param {Object} navigateState navigateState object
 */
function resetTree( vmCollection, selectionModel, classifyState, deselect, searchString, navigateState ) {
    var response = {};
    response.reset = true;
    ...
    if( navigateState && navigateState.value ) {
        clsTreeSvc.updateSelectedClassNode( null, navigateState, response, searchString );
    } else {
...
function resetSelectedToDefault( response, context ) {
    ...
    if ( selected && selected.length && treeResult.parentNode && selected[0].uid === treeResult.parentNode.uid ||
        ...
        if ( response.reset ) {
            ...
            tmpContext.selectedClassNode = null;
