...
export let processCellSelection = function( data, classifyState, responseState ) {
    ...
    data.nodeAttr = null;
    let prevSelectedUid = classifyState.value.selectedClass ? classifyState.value.selectedClass.uid : '';
    ...
    if( data.selectedCell && data.selectedCell.selected ) {
        ...
        } else if( data.icoCells && data.icoCells.length > 0 && ( prevSelectedUid === '' || prevSelectedUid === data.selectedCell.uid ) && data.cancelEditAction !== true ) {
            classifyNodeSvc.formatDataAndResponse( appCtxSvc.getCtx( 'ICO_response' ), data, classifyState, responseState );
        } else {
