...
/**
 * Returns owning site property
 * @param {Object} wso Workspace object
 * @returns dbvalue for owning site property
 */
export let checkForProps = function( wso ) {
    if( wso && wso[0] && wso[0].props && wso[0].props.owning_site ) {
        return wso[0].props.owning_site.dbValue;
    }
};
/*
 ...
 */
export let generateCells = function( response, responseState, workspaceObjectName ) {
    var cells = [];
    ...
    if( response && response.clsObjectDefs && !_.isEmpty( response ) ) {
        ...
        _.forEach( response.clsObjectDefs[ 1 ][ 0 ].clsObjects, function( clsObj ) {
            ...
            //object_string for workspace object
            if ( workspaceObjectName ) {
                vemo.props.icoName = uwPropertyService.createViewModelProperty( 'icoName', 'ICO NAME', 'STRING', workspaceObjectName, workspaceObjectName );
            }
    ...
    if( responseState ) {
        const tmpState = { ...responseState.value };
        tmpState.response = response;
        responseState.update( tmpState );
    }
 ...
 * @param {Object} data - view model data
 * @param {Boolean} isPoly - true if polymorphic node, false otherwise
 * @param {Object} isValidData - validData
 *
 */
export let validateProps = function( node, isRequiredAttrsArray, data, isPoly, isValidData ) {
    if( node.type !== 'Block' ) {
    ...
    if( node.polymorphicTypeProperty && !node.cardinalController ) {
        isValidData = exports.validateProps( node.polymorphicTypeProperty, isRequiredAttrsArray, data, true, isValidData );
    ...
    if( node.cardinalController ) {
        ...
        for( var i = 0; i < node.instances.length; i++ ) {
            ...
            if( instance.polymorphicTypeProperty ) {
                isValidData = exports.validateProps( instance.polymorphicTypeProperty, isRequiredAttrsArray, data, true, isValidData );
                if( !isValidData.isValidtoSave ) {
                    if( node.tableView ) {
            ...
            isValidData = exports.validateProps( instance, isRequiredAttrsArray, data, false, isValidData );
            if( !isValidData.isValidtoSave ) {
                break;
            }
        }
        ...
        for( var ii = 0; ii < node.children.length; ii++ ) {
            isValidData = exports.validateProps( node.children[ ii ], isRequiredAttrsArray, data, false, isValidData );
            if( !isValidData.isValidtoSave ) {
                break;
            }
        }
...
export let onSaveButtonValidation = function( data, classifyState ) {
    ...
    for( var i = 0; i < data.attr_anno.length; i++ ) {
        isValidData = exports.validateProps( data.attr_anno[ i ], isRequiredAttrsArray, data, false, isValidData );
        if( !isValidData.isValidtoSave ) {
            break;
        }
        if( data.eventData && data.eventData.goToView ) {
