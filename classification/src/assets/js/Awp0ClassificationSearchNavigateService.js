...
/**
  ...
  */
export let activateUpHierarchy = function( navigateState, searchState, selectionData ) {
    const tmpNavContext = { ...navigateState.value };
    tmpNavContext.upHierarchy = true;
    if( navigateState.parent === 'top' ) {
        tmpNavContext.selectedNode = '';
        let tmpSelContext = { ...selectionData.value };
        tmpSelContext.selected = [];
        selectionData.update( tmpSelContext );
        const tmpContext = { ...searchState.value };
        if( tmpContext.reset ) {
            delete tmpContext.reset;
        }
...
/**
  * Navigates up the class hierarchy to a parent. Replaces the selected node with the parent ID and indicates an update is happening.
  * @param {State} navigateState the current navigation state for the classification location navigator tab.
  * @param {State} searchState the search state to update with the new class to search by, if available.
  * @param {State} selectionData the selectionData to clean out, if we are at the top.
  */
export let respondToUpHierarchy = function( navigateState, searchState, selectionData ) {
    if( navigateState.upHierarchy ) {
        const tmpNavContext = { ...navigateState.value };
        tmpNavContext.selectedNode = {
        ...
        tmpNavContext.upHierarchy = false;
        navigateState.update( tmpNavContext );
    }
};
...
export let toggleNavigateTree = function( commandContext, showNavigateImage ) {
    ...
    //Clear out search criteria if there is a selection.
    if( commandContext.selectionData.selected && commandContext.selectionData.selected.length ) {
        tmpContext.searchCriteria = null;
        tmpContext.selectedNode = commandContext.selectionData.selected[0];
    }
...
export let determineBreadcrumb = function( current, navigateState ) {
    if( navigateState ) {
        if( navigateState.selectedNode && navigateState.selectedNode.uid !== classifyDefinesService.ROOT_LEVEL_ID  ) {
            return false;
        }
        if( navigateState.selectedNode && navigateState.selectedNode.uid === classifyDefinesService.ROOT_LEVEL_ID  ) {
            return true;
        }
        if ( !navigateState.value.showNavigateImage ) {
            return false;
        }
        if ( !navigateState.parent ) {
            return true;
        }
    }
    return current;
...
export let updateSearchLocation = function( selectionData, searchState, navigateState ) {
    ...
    if ( selectionData.value && selectionData.value.selectedClassNode && selectionData.value.selectedClassNode.uid ) {
