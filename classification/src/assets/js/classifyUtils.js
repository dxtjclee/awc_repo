 ...
 */
function ClsUtilValueMap( data, classID, properties, blockProperties, attributeArray, classifyState ) {
    ...
    if( properties || blockProperties ) {
        ...
        this.properties = this.getRootLevelPropertiesFromAttributes( attributeArray, data, classifyState );
        this.blockProperties = this.getBlocksFromAttributes( attributeArray, data.unitSystem, data, classifyState );
    }
}

let updateAttribute = function( attributeVmo ) {
...
let updateAttributeArray = function( data, classifyState, attributeArray, attributeVmo, isNullOrEmptyDbValue ) {
    //get other attribute and pass the same
    _.forEach( attributeArray, function( interDepAttr ) {
        if( interDepAttr.attrDefn.isInterDependentKLAttribute ) {
            if( !isNullOrEmpty && attributeVmo.valueUpdated  ) {
                //if value exists , lets use the same for updating other values

                var propDisplayValue;
                var propInternalValue;
                if( interDepAttr.attrDefn.dbValue && interDepAttr.attrDefn.dbValue.length > 0 ) {
                    propDisplayValue = interDepAttr.attrDefn.dbValue[0];
                    propInternalValue = interDepAttr.attrDefn.displayValue[0];
                } else if( isNullOrEmptyDbValue ) {
                    propDisplayValue = '';
                    propInternalValue = '';
                }
                var selected = [
                ...
                classifyLOVService.validateLOVValueSelections( data, interDepAttr, classifyState, selected );
            }
        }
    } );
};

let updateComplexAttribute = function( attribute, values ) {
...
let updateDateAttribute = function( data, attribute, attributeVmo, dbValue, item  ) {
    ...
    if( dbValue === exports.CLEARED_WIDGET_DATE_CHROME ||
        ...
        var tempUnitSystem = {};
        if( data.unitSystem && data.unitSystem.dbValue ) {
            tempUnitSystem = data.unitSystem.dbValue;
        } else {
        ...
        var formatTypeIndex = exports.CST_DEFAULT_DATE_FORMAT_INDEX;
        if ( tempAttrId.substring( 0, 4 ) === 'cst0' || tempAttrPrefix.substring( 0, 4 ) === 'cst0' ) {
            formatTypeIndex = exports.CST_DATE_FORMAT_INDEX;
        } else if ( tempUnitSystem.formatDefinition ) {
            formatTypeIndex =  tempUnitSystem.formatDefinition.formatLength;
        }
...
let updateUnitAttribute = function( attribute, attributeVmo, valIdx  ) {
    var propertyValueObj = {
        internalValue: '',
        displayValue: ''
    };
    ...
    if ( attributeVmo.dbValues ) {
        ...
        } else if ( attributeVmo.displayValues && attributeVmo.displayValues.length > 0 ) {
 ...
 */
ClsUtilValueMap.prototype.getRootLevelPropertiesFromAttributes = function( attributeArray, data, classifyState  ) {
    ...
    _.forEach( attributeArray, ( attribute ) => {
        ...
        if( attribute.type !== 'Block' && attribute.type !== 'Separator' ) {
            ...
            var isNullOrEmptyDbValue = exports.isNullOrEmpty( attributeVmo.dbValue );

            if ( attributeVmo.valueUpdated || !isNullOrEmptyDbValue ) {
                updateAttribute( attributeVmo );
            }

            if( attributeVmo.valueUpdated && isNullOrEmptyDbValue
             &&  attribute.attrDefn && attribute.attrDefn.isInterDependentKLAttribute ) {
                //special fix for interdependency
                updateAttributeArray( data, classifyState, attributeArray, attributeVmo, isNullOrEmptyDbValue );
            }

            if ( attribute.vmps.length > COMPLEXVMOMINIMUMLENGTH ) { //Complex
                updateComplexAttribute( attribute, values );
                ...
                //for boolean attribute, assigning "true" or "false" value to attribute in non-english locale if boolean value is in corresponding locale
                if( localeSvc.getLocale() !== 'en_US' && attributeVmo.type === 'BOOLEAN' ) {
                    assignEnglishValueToBooleanAttribute( attributeVmo );
                }
                ...
                attributeVmo.displayValues.forEach( ( item, valIdx ) => {
                    ...
                    if( attributeVmo.dbValue ) {
                        if ( _.isArray( attributeVmo.dbValue ) && attributeVmo.dbValue.length > 0 ) {
                            dbValue =  valIdx < attributeVmo.dbValue.length ? attributeVmo.dbValue[ valIdx ] : '';
                        } else {
                            dbValue = attributeVmo.dbValue.toString();
                        }
                    }

                    if( dbValue && attributeVmo.type === 'DATE' || attributeVmo.type === exports.DATE_ARRAY ) {
                        propertyValueObj = updateDateAttribute( data, attribute, attributeVmo, dbValue, item  );
                        ...
                        if( attributeVmo && attributeVmo.displayValues && attributeVmo.displayValues[ valIdx ] ) {
                            ...
                            if( attribute.vmps && attribute.vmps[ 2 ] && ( attribute.vmps[ 2 ].names || attribute.vmps[ 2 ].propertyName ) ) {
                                propertyValueObj = updateUnitAttribute( attribute, attributeVmo, valIdx  );
...
function parseValues( values, attributeVmo, attribute, rootLevelProperties ) {
    ...
    if ( hasEmptyVals && attributeVmo.hasLov && ( attribute.isArray && attributeVmo.prevDisplayValues.length > 0 ) ) {
        hasEmptyVals = false;
    }
 ...
 */
ClsUtilValueMap.prototype.getBlocksFromAttributes = function( attributeArray, unitSystem, data, classifyState ) {
    var blockProperties = {};
            ...
            if( attribute.cardinalController ) {
                for( var instanceInx in attribute.instances ) {
                    ...
                    var instanceObject = {
                        ...
                        properties: this.getRootLevelPropertiesFromAttributes( instance.children, data, classifyState ),
                        blockDataMap: this.getBlocksFromAttributes( instance.children, unitSystem, data, classifyState )
                    };
                ...
                var blockObject = {
                    ...
                    properties: this.getRootLevelPropertiesFromAttributes( attribute.children, data, classifyState  ),
                    blockDataMap: this.getBlocksFromAttributes( attribute.children, unitSystem, data, classifyState )
                };
 ...
 */
export let getClsUtilValueMap = function( data, classID, properties, blockProperties, attributeArray, classifyState ) {
    var objectOrNull = null;
    //At least one of these arguments must not be null
    if( properties || blockProperties || attributeArray ) {
        objectOrNull = new ClsUtilValueMap( data, classID, properties, blockProperties, attributeArray, classifyState );
    }
    return objectOrNull;
};
...
export let formatImageAttachments = function( datasetFilesOutput ) {
    ...
    if( datasetFilesOutput && datasetFilesOutput.length > 0 ) {
        ...
        _.forEach( datasetFilesOutput, function( dsOutputArrElement ) {
            ...
            var type = dsOutputArrElement.documentType;
            var n = ticket.lastIndexOf( '.' );
            var ticketExt = ticket.substring( n + 1 ).toUpperCase();
            if( exports.isSupportedImageType( ticket ) && type === 'image' ) {
 ...
 * Returns an integer of a string attribute id by removing the prefix from the beginning of it, if has one
 *
 * @param {Object} attributesDefinition - The string attributeId to convert to integer
 * @returns {Boolean} attribute
 */
export let checkIfAttributeIsBlockorFrame = function( attributesDefinition ) {
    return exports.getPropertyValueFromArray( attributesDefinition.attributeProperties,
        classifyDefinesService.UNCT_ATTRIBUTE_TYPE ) === classifyDefinesService.CLS_PROPERTY_ATTR_TYPE_FRAME || exports.getPropertyValueFromArray( attributesDefinition.attributeProperties,
        classifyDefinesService.UNCT_ATTRIBUTE_TYPE ) === classifyDefinesService.CLS_PROPERTY_ATTR_TYPE_BLOCK;
};
...
/**
 * Assigns Boolean attribute with english value(true/false) if in non-english locale
 *
 * @param {Object} attributeVmo - attribute's vmo
 */
export const assignEnglishValueToBooleanAttribute = function( attributeVmo ) {
    if( attributeVmo && attributeVmo.displayValues ) {
        if( attributeVmo.displayValues[0] === attributeVmo.propertyRadioTrueText ) {
            attributeVmo.displayValues[0] = classifyDefinesService.LOGICAL_STR_TRUE;
            attributeVmo.uiValue = classifyDefinesService.LOGICAL_STR_TRUE;
        } else if( attributeVmo.displayValues[0] === attributeVmo.propertyRadioFalseText ) {
            attributeVmo.displayValues[0] = classifyDefinesService.LOGICAL_STR_FALSE;
            attributeVmo.uiValue = classifyDefinesService.LOGICAL_STR_FALSE;
        }
    }
};
...
export default exports = {
    ...
    getUnitSystem,
    getParentIds,
    checkIfAttributeIsBlockorFrame,
