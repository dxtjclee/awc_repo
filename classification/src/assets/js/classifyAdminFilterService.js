...
import classifyAdminConstants from './classifyAdminConstants';
...
export let addFilter = function( data, addFilterList ) {
    addFilterList.dbValue.push( {
        ...
        operationTypeValuesBasic: data.operationTypeValuesBasic,
        operationTypeValuesNodes: data.operationTypeValuesNodes,
 ...
 * @param {Boolean} advancedSystem whether we're using the Advanced class system or not.
 * @returns {Object} Listbox, textbox list and type
 */
export let prepareList = function( subPanelContext, advancedSystem ) {
    let item = { ...subPanelContext.item };
    let operationType = item.operationType;
    let operationTypeValues = advancedSystem ? item.operationTypeValues : item.operationTypeValuesBasic;
    let operationValues = item.operationValues;
    if( subPanelContext.pageContext.primaryActiveTabId === classifyAdminConstants.NODES ) {
        operationTypeValues = item.operationTypeValuesNodes;
    }
