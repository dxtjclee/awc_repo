...
export let showPropGroups = function( commandContext ) {
    ...
    // In classify panel, if deselected, show properties by default.
    if( !tmpState.showPropTree ) {
...
export let initializeTab = ( classifyState, classifyStateUpdater ) => {
    ...
    //Ensure clsadmin is unregisted, causing issue with block expansion
    appCtxSvc.unRegisterCtx( 'clsadmin' );
    const updateAtomicData = classifyStateUpdater.classifyState;
...
export let resetClassifyState = function( classifyState ) {
    ...
    classifyState.standAlone = false;
 ...
 * @return {*} classifyState classify state
 */
export let resetCreateMode = function( classifyState, searchState  ) {
    let tmpState = { ...classifyState.value };
    tmpState = resetClassifyState( tmpState );
    classifyState.update( tmpState );
    let tmpSearchState = { ...searchState };
    if ( tmpSearchState.standAlone === true ) {
        tmpSearchState.standAlone = undefined;
