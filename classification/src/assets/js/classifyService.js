...
export let ATTRIBUTE_TYPE = 'ATTRIBUTE_TYPE';
...
//I18n constants
export let IS_ARRAY_SIZE_CONTROLLER = 'IS_ARRAY_SIZE_CONTROLLER';
export let KEYLOV_IRDI_METRIC = 'KEYLOV_IRDI_METRIC';
export let REFERENCED_CLASS_IRDI = 'REFERENCED_CLASS_IRDI';
...
export let ATTRIBUTE_NAME = 'ATTRIBUTE_NAME';
export let ATTRIBUTE_SHORT_NAME = 'ATTRIBUTE_SHORT_NAME';
export let ATTRIBUTE_ANNOTATION = 'ATTRIBUTE_ANNOTATION';
export let ATTRIBUTE_CONFIG_BEFORE = 'ATTRIBUTE_CONFIG_BEFORE';
export let ATTRIBUTE_CONFIG = 'ATTRIBUTE_CONFIG';
export let ATTRIBUTE_CONFIG_AFTER = 'ATTRIBUTE_CONFIG_AFTER';
export let ATTRIBUTE_DEPENDENCY_CONFIG = 'ATTRIBUTE_DEPENDENCY_CONFIG';
export let ATTRIBUTE_DEPENDENCY_ATTRIBUTE = 'ATTRIBUTE_DEPENDENCY_ATTRIBUTE';
export let ATTRIBUTE_IS_LAYOUT = 'ATTRIBUTE_IS_LAYOUT';

export let ATTRIBUTE_TYPE_METRIC = 'ATTRIBUTE_TYPE_METRIC';
export let ATTRIBUTE_TYPE_NONMETRIC = 'ATTRIBUTE_TYPE_NONMETRIC';
export let UNIT_SYSTEM_METRIC = 'UNIT_SYSTEM_METRIC';
export let UNIT_SYSTEM_NONMETRIC = 'UNIT_SYSTEM_NONMETRIC';


export let ARR_METADATA_NODE_APP_CLASS_JSON_SML_CLASS_PROP = [
    UNCT_CLASS_NAME,
    UNCT_CLASS_ID,
    UNCT_CLASS_UNIT_SYSTEM
];

export let ARR_METADATA_NODE_APP_CLASS_JSON_SML_CLASS_PROP_DISP = [
    UNCT_CLASS_NAME_DISP,
    UNCT_CLASS_ID_DISP,
    UNCT_CLASS_UNIT_SYSTEM_DISP
];

export let UNCT_ATTR_PROP_I18N_DISP = [
    ATTRIBUTE_NAME,
    ATTRIBUTE_TYPE_METRIC,
    ATTRIBUTE_TYPE_NONMETRIC,
    ATTRIBUTE_SHORT_NAME,
    ATTRIBUTE_ANNOTATION,
    ATTR_CARDINALITY,
    UNIT_SYSTEM_METRIC,
    UNIT_SYSTEM_NONMETRIC
];
...
export let UNCT_ATTR_PROP_CST_DISP = [
    ATTRIBUTE_TYPE,
    ATTRIBUTE_NAME,
    ATTRIBUTE_SHORT_NAME,
    ATTRIBUTE_ANNOTATION,
    REFERENCED_CLASS_IRDI,
    IS_ARRAY_SIZE_CONTROLLER,
    ATTR_CARDINALITY,
    KEYLOV_IRDI_METRIC,
    UNIT_SYSTEM_METRIC,
    UNIT_SYSTEM_NONMETRIC
        ...
        if( attr.vmps[ '0' ].hasLov ) {
            ...
            if( KeyLovArray.length > 0 ) {
                ...
                if( attr.vmps[ '0' ].displayValues.length > 0 && attr.vmps[ '0' ].displayValues[ '0' ] !== '' ) {
                    ...
                    if( !keyLovs.includes( attr.vmps[ '0' ].displayValues[ '0' ].trim() ) ) {
                        isKeyValid = false;
 ...
 * @returns {String} imageIconUrl String representation of icon
 */
 ...
 */
export let getDependentKeyLOVAttributes = function( attributesDefinitions, blockDefinitionMapResponse ) {
    ...
    if( attributesDefinitions ) {
        _.forEach( attributesDefinitions, function( attributesDefinition ) {
            if( attributesDefinition.attributeKeyLOVDef ) {
                //Verify whether attribute is block/frame if yes get ids of attribute within particular block/frame
                if( classifyUtils.checkIfAttributeIsBlockorFrame( attributesDefinition ) ) {
                    setAttributeIdsFromBlock( blockDefinitionMapResponse[ attributesDefinition.attributeId ].attributes, configKeyLOVAttributeIds );
                }
...
/*
 * Stores attribute ids from property blocks
 * @param {Object} attributes - list of attributes/blocks in a block
 * @param {Object} configKeyLOVAttributeIds - stores attribute ids.
 */
let setAttributeIdsFromBlock = function( attributes, configKeyLOVAttributeIds ) {
    _.forEach( attributes, function( attribute ) {
        if ( classifyUtils.checkIfAttributeIsBlockorFrame( attribute ) ) {
            setAttributeIdsFromBlock( attributes, configKeyLOVAttributeIds );
        }
        if( attribute.attributeKeyLOVDef && attribute.attributeKeyLOVDef.keyLOVEntries.length > 0 ) {
            //Currently, we need to transform ID into an integer. In future implementation, when the dependentKeyLov
            //SOA is replaced, we will need to use this as a string
            configKeyLOVAttributeIds.push(
 ...
 */
export let getReleaseDisplayName = function( struct, name, totalReleases ) {
    ...
    if( struct && struct.length !== 0 ) {
        ...
        } else if( totalReleases ) {
            /*
                The release name is not found in the struct, therefore search the name in
                totalRelease,
            */
            var newIdx = _.findIndex( totalReleases, function( release ) {
                return name === release;
            } );

            if( newIdx !== -1 ) {
            /*
               To solve, we have passed an additional parameter which contains details of display name and internal names of the releases in form of array.
               e.g. totalReleases [ ] = "Internal Name1", "Display Name1","Internal Name2", "Display Name2","Internal Name3", "Display Name3"......
               Check the size of totalRelease should be <= new index generated.
            */
                var tempIdx = newIdx + 1;
                if( tempIdx <= totalReleases.length ) {
                    displayName = totalReleases[tempIdx];
                }
            }
        }
    }
...
export let setDefaultValuesAndUnitSystem = function( data, attributeArray, isMetricUnitSystem, classifyState ) {
    _.forEach( attributeArray, function( attribute ) {
        try {
            ...
            if( attribute.type === 'Block' ) {
                exports.setDefaultValuesAndUnitSystem( data, attribute.children, isMetricUnitSystem, classifyState );
                if( !classifyUtils.isNullOrEmpty( attribute.instances ) ) {
                    exports.setDefaultValuesAndUnitSystem( data, attribute.instances, isMetricUnitSystem, classifyState );
                }
            } else {
        ...
        if( cardinalValue > blockAttribute.instances.length ) {
            for( var i = blockAttribute.instances.length; i < cardinalValue; ++i ) {
                ...
                if( blockAttribute.polymorphicTypeProperty ) {
                    ...
                    tempArr.polymorphicTypeProperty.vmps[0].level = blockAttribute.level + 1;
                }
                ...
                _.forEach( tempArr.children, function( child ) {
                    if ( child.polymorphicTypeProperty ) {
                        ...
                        child.polymorphicTypeProperty.vmps[0].level = child.level + 1;
                    }
                ...
                //Collapse all instances to improve performance
                tempArr.propExpanded = false;
...
export let getViewProps = function( data, formattedAttribute, valuesMap, currentUnitSystem, indexMap ) {
    ...
    if( currentUnitSystem.formatDefinition.formatType > COMPLEXPROPERTY && dbValue && dbValue.length > 1 ) {
        ...
        formattedAttribute.vmps = exports.formatViewProps( data, formattedAttribute.origAttributeInfo, formattedAttribute.attrDefn, dbValue, null, null, valuesMap );
    }
};
...
/**
 * Returns ViewModel for Attribute's Unit
 * Check whether Attribute's vmp already has new unit value if yes return same else create ViewProp for unit
 * @param {Object} attrDefn Attribute Definition
 * @param {Object} originalVmo Attribute ViewModel Props
 * @return {Object} viewprop for unit
 */
function getViewPropUnitSystem( attrDefn, originalVmo ) {
    if( attrDefn.unitSystem && originalVmo && originalVmo[2] && attrDefn.unitSystem.unitName === originalVmo[2].uiValue ) {
        return originalVmo[2];
    }
    return attrDefn.createViewPropUnitSystem();
}
...
export let formatViewProps = function( data, attribute, attrDefn, dbValue, attrOptions, originalVmo, valuesMap, isAdmin ) {
    ...
    if( ( ( attribute.options & exports.ATTRIBUTE_HIDDEN ) !== 0 ||
        ...
        ( attribute.options & exports.ATTRIBUTE_REFERENCE ) !== 0 && data.panelMode !== -1  ) && !isAdmin ) {
    ...
    var viewProp3 = getViewPropUnitSystem( attrDefn, originalVmo );
...
export let fetchApplicableValuesMap = function( valuesMap, attr ) {
    if( valuesMap && valuesMap.blockProperties ) {
        //Fix for eClass 11.1
        let idTokens = attr.idPrefix.split( '.' );
        let parent_prefix;
        if ( idTokens.length === 0 ) {
            idTokens = attr.idPrefix.split( '-' );
        }
        let idTokensLength = idTokens.length;
        if ( idTokensLength > 0 ) {
            parent_prefix = idTokens[ idTokensLength - 1 ];
            if ( idTokens.length === 1 ) {
                //top level contains cst0 prefix
                parent_prefix = parent_prefix.substring( 4 );
            } else if ( parent_prefix === '' ) {
                parent_prefix = idTokens[ idTokensLength - 2 ];
            }
            let blockDataMap = valuesMap.blockProperties.blockDataMap;
            for ( var idx = 0; idx < idTokensLength - 1; idx++ ) {
                parent_prefix = idTokens[ idx ];
                if( parent_prefix.substring( 0, 4 ) === CSTPREFIX ) {
                    parent_prefix = parent_prefix.substring( 4, parent_prefix.length );
                }
                if ( blockDataMap && blockDataMap[ parent_prefix ]  ) {
                    let blocks = blockDataMap[parent_prefix].blocks;
                    if ( blocks.length === 0 ) {
                        //no more nested blocks
                        return null;
                    }
                    if ( idx === idTokensLength - 2 ) {
                        //last node
                        return blocks[0];
                    }
                    //check nested block
                    blockDataMap = blocks[0].blockDataMap;
                }
            }
        }
    }
...
export let formatAttributeArrayForAdmin = function( data, attributeArray, valuesMap, outputAnnoArray, attributeIDPrefix, isPanel, formatArrSizeContr,
    ...
    _.forEach( attributeArray, function( attributeDefinition ) {
        ...
        attr.setDbValue( data );
        var entry = attr.getAttributeEntry( data, attribute, attrOptions, true );
...
let getPolyBlockAttributes = function( data, responseState, propValue, propLevel, owningAttribute, values, attrPrefix ) {
    ...
    if( propValue && blockDefinitionMap ) {
        ...
        if ( propValue !== data.i18n.select && blockDefinitionMap.hasOwnProperty( propValue ) && blockDefinitionMap[ propValue ] ) {
            ...
            _.forEach( childBlock, function( block ) {
                block.level = propLevel + 1;
            } );
            exports.hasBlockChildren( childBlock );
            owningAttribute.hasBlockChildren = childBlock.length > 0;
...
export let selectPolyLOV = function( data, responseState, lovEntry, owningAttribute ) {
    ...
    lovEntry.level = lovEntry.level ? lovEntry.level : owningAttribute.level;
...
export let hasBlockChildren = function( propertyGroup ) {
    if( propertyGroup ) {
        if ( _.isArray( propertyGroup ) ) {
            _.forEach( propertyGroup, function( propGrp ) {
                exports.hasBlockChildren( propGrp );
            } );
        } else {
...
function Attribute( data, attribute, attributeIDPrefix, isPanel ) {
    ...
    this.attrTypeForAdmin = this.attrType;
    this.formatLengthMetric = attribute.metricFormat.formatDefinition.formatLength;
    this.formatLengthNonMetric = attribute.nonMetricFormat.formatDefinition.formatLength;
    ...
    if( this.unitSystem.formatDefinition.formatType > 0 && this.unitSystem.formatDefinition.formatType < 3 ) {
        //compute the format
        this.formatLengthMetric = attribute.metricFormat.formatDefinition.formatLength - ( attribute.metricFormat.formatDefinition.formatModifier2 + 1  ) + '.' + attribute.metricFormat.formatDefinition.formatModifier2;
        this.formatLengthNonMetric = attribute.nonMetricFormat.formatDefinition.formatLength - ( attribute.nonMetricFormat.formatDefinition.formatModifier2 + 1  ) + '.' + attribute.nonMetricFormat.formatDefinition.formatModifier2;
    }
    if( this.unitSystem.formatDefinition.formatType === 3  ) {
        //compute the format
        //Date:To-Do Revisit
        this.formatLengthMetric = attribute.metricFormat.formatDefinition.formatLength;
        this.formatLengthNonMetric = attribute.nonMetricFormat.formatDefinition.formatLength;
    ...
    this.unitSystemMetric = !_.isEmpty( attribute.metricFormat.unitName ) ? attribute.metricFormat.unitName :  '';
    this.unitSystemNonMetric = !_.isEmpty( attribute.nonMetricFormat.unitName ) ? attribute.nonMetricFormat.unitName : '';
 ...
 */
Attribute.prototype.getAttributeEntry = function( data, attribute, attrOptions, isAdmin ) {
    var vmoArray = exports.formatViewProps( data, attribute, this, null, attrOptions, null, null, isAdmin );
...
Attribute.prototype.setLOVValue = function( data, attribute, dbValue, displayValue, valuesMap ) {
    ...
    if ( dbValue && displayValue.length === 0 ) {
        _.forEach( valuesMap.properties, function( value ) {
            if( value.propertyId === attribute.attributeId ) {
...
Attribute.prototype.createViewPropType = function( data, attribute, attrOptions, attrLevel ) {
    ...
    viewProp.isFixedValueAttr = isFixedValueAttribute( attribute ); /* fixed/protected/hidden/referenced */
    ...
    if( this.isLOV ) {
        ...
        if( this.isInterDependentKLAttribute ) {
            ...
            var dependentAttributeIds = exports.getDependentKeyLOVAttributes( attributeArray, data.blockDefinitionMapResponse );
...
/*
 * Gets the formatted filter key which will be compatible for performSearchViewModel5
...
export let modifyForReleaseName = function( releasesState, node, nodeName ) {
    ...
    let totalReleases = releasesState ? releasesState.value.totalReleases : null;
    if ( releasesStruct && releasesStruct.length > 1 ) {
        var standard = getPropertyValue( node.properties, 'SOURCE_STANDARD' );
        if( standard && standard !== '' ) {
            var displayName = getReleaseDisplayName( releasesStruct, standard, totalReleases );
...
/*
 * Returns true if attribute in a class is configured to be hidden,
 * protected, fixed or referencing attributes from other related objects
 *
 * @param {attr} attribute to be checked if editable
 * @returns true if attribute is user editable
 */
let isFixedValueAttribute = function( attr ) {
    var isFixedValue = false;
    // If attribute is hidden, read-only(fixed), or reference attribute.
    if( ( attr.options & exports.ATTRIBUTE_HIDDEN ) !== 0 ||
        ( attr.options & exports.ATTRIBUTE_PROTECTED ) !== 0 ||
        ( attr.options & exports.ATTRIBUTE_REFERENCE ) !== 0 ||
        ( attr.options & exports.ATTRIBUTE_FIXED ) !== 0 ||
        ( attr.options & exports.ATTRIBUTE_FIXED2 ) !== 0 ) {
        isFixedValue = true;
    }
...
export default exports = {
    ...
    LOAD_CLASS_UNITS,
    ARR_METADATA_NODE_APP_CLASS_JSON_SML_CLASS_PROP,
    ARR_METADATA_NODE_APP_CLASS_JSON_SML_CLASS_PROP_DISP,
    ATTR_CARDINALITY,
    ATTRIBUTE_SHORT_NAME,
    ATTRIBUTE_CONFIG_BEFORE,
    ATTRIBUTE_CONFIG,
    ATTRIBUTE_CONFIG_AFTER,
