...
/**
  ...
  * @param {Object} navigateState the state keeping track of the search box.
  */
export let clearSearch = function( searchBox, externalSearchBox, navigateState ) {
    searchBox.dbValue = '';
    externalSearchBox.dbValue = '';
    let tmpContext = { ...navigateState.value };
    if( tmpContext.searchCriteria ) {
        tmpContext.searchCriteria = '';
    ...
    return function() {
        if ( searchCriteria.length >= 1 &&
            ...
            if ( searchState && searchCriteria && !isPanel ) {
                ...
                tmpSearchContext.selectedClassNode = null;
 ...
 */
export let updateSearchStateSyncCrumbObj = function( selectedObj, searchState ) {
    if( searchState ) {
        const tmpSearchState = { ...searchState.value };
        tmpSearchState.crumbSyncObj = selectedObj;
