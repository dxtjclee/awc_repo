 ...
 * @param {Object} attrDepAttribute dependent attribute
 */
function ClsLovApi( data, classID, attributeID, keyLOVDefinition, isInterDependentKeyLOV, dependentAttributeIds,
    ...
    if ( viewProp && viewProp.dbValue !== null && viewProp.dbValue[0] ) {
        ...
        data.dAttributeStruct.push( {
            classID: lovSvc ? lovSvc.classID : '',
            selectedAttributeID: lovSvc ? lovSvc.attrID : '',
    ...
    if ( keyLOVDefinition && keyLOVDefinition.keyLOVEntries ) {
        ...
        for ( ; localIndex < keyLOVDefinition.keyLOVEntries.length; localIndex++ ) {
            ...
            if ( exports.isSubmenuStart( subStr ) ) {
                ...
                let displayValue = keyLOVDefinition.keyLOVOptions === 1 ? keyLOVEntry.keyLOVValue :
                    keyLOVEntry.keyLOVkey + ' ' + keyLOVEntry.keyLOVValue;
                subMenuRegObject.propDisplayValue = displayValue.trim();
...
export let initializeInterdepLOV = function( data, attr, classifyState ) {
    ...
    if ( attr.fielddata.uiValue === '' ) {
        ...
        _.forEach( attr.fielddata.lovApi.dependentAttributeIds, function( attrDep ) {
            if (  viewPropMap[attrDep] && viewPropMap[attrDep].dbValue !== '' ) {
                hasValue = true;
            }
        } );
...
export let getDependencyAttributeStruct = function( data, attr, classifyState ) {
    ...
    _.forEach( list, function( listItem ) {
        var element = getAttrInLovSet( tmpState.attrs, listItem );
        if ( element ) {
...
export let getResponseForInitialization = function( response, data, attr, classifyState, selected ) {
    ...
    var element = getAttrInLovSet( tmpState.attrs, attr.fielddata.lovApi.attrID );
    element.vmps[0].dbValue = [];
    if ( selected ) {
    ...
    if ( response ) {
        ...
        if ( response.dependencyKeyLOVs ) {
            ...
            _.forEach( response.dependencyKeyLOVs, function( depKLEntryFromResp ) {
                ...
                _.forEach(
                    ...
                    function( entry ) {
                        ...
                        if ( entry.lovKey === depKLEntryFromResp.selectedKeys[0] ) {
                            ...
                            const element = getAttrInLovSet( tmpState.attrs, depKLEntryFromResp.attributeID );
                            if ( element ) {
                                let lovEntry = {
                                    propInternalValue: entry.lovKey,
                ...
                var attrElement = getAttrInLovSet( tmpState.attrs, depKLEntryFromResp.attributeID );
...
/**
 * Returns attribute from attrSet with attribute ID "attrID"
 *
 * @param {Array} attrSet set of attributes
 * @param {Integer} attrID attribute selected
 * @returns {Object} attribute with particular attribute ID (attrID)
 */
let getAttrInLovSet = function( attrSet, attrID ) {
    var vmpattr;
    _.forEach( attrSet, function( attrNew ) {
        if( attrNew.type === classifyDefinesService.CLS_PROPERTY_TYPE_BLOCK ) {
            var attr = getAttrInLovSet( attrNew.children, attrID );
            if( attr && attr.id !== undefined ) {
                vmpattr = attr;
            }
        }
        if( attrID === getAttributeIntegerFromString( attrNew.id ) &&  attrNew.type !== classifyDefinesService.CLS_PROPERTY_TYPE_BLOCK ) {
            vmpattr = attrNew;
        }
    } );
    return vmpattr;
...
let initAttrAtIndexInLovSet = function( attrSet, attrID ) {
    var element = getAttrInLovSet( attrSet, attrID );
    if ( element ) {
