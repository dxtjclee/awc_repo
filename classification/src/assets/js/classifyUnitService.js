...
var convertVMO = function( attribute, conversionUnit, vmp ) {
    try {
        ...
        let attrValue = vmp.prevDisplayValues;

        //Checking if the previous value is blank then go with the current display value
        //else part covers when the value is converted back to metric format.

        if( attrValue.length === 0 ) {
            //conversion unit false : metric to non-metric conversion
            if( !conversionUnit ) {
                attrValue = vmp.dbValue;
            } else {
                attrValue = vmp.newValue;
            }
        }
        ...
        if ( !conversionUnit ) {
            ...
            if( vmp.nonMetricFormat.startValue !== undefined ) {
...
let applyConvertedValue = function( attribute, convertedValue, panelMode, unitSystem, val ) {
    ...
    if ( panelMode !== -1 ) {
        if ( attribute.vmps[0].isFixedValueAttr ) {
