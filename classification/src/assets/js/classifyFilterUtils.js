...
export let filterHierarchy = function( data, searchString, providerName, conditions ) {
    ...
    if( data.dataProviders[providerName].initializationComplete || searchString && searchString.length >= 1 && conditions.isValidSearchInput ) {
        if(  !conditions.syncPanel || conditions.selectionCanBeCleared  ) {
