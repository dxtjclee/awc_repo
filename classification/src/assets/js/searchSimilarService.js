...
let createTreeVmNode = function( node, level, isLeaf ) {
    var vmNode = awTableTreeSvc.createViewModelTreeNode( node.dbValue, '', node.displayValues, level, 0, '' );
...
export let getFilterMapForSearchSimilar = function() {
    ...
    for( var key in classSearchSimilar ) {
        ...
        if( classSearchSimilar[ key ].formatType === 3 ) {
            ...
            var dateValue = '';
            if( classSearchSimilar[ key ].values && classSearchSimilar[ key ].values.length > 0 ) {
                dateValue = classifyUtils.convertClsDateToAWDateWidgetFormat(
                    classSearchSimilar[ key ].values[ 0 ].internalValue, classSearchSimilar[ key ].formatLength, false ).dbValue;
                catFilterValuesForSearchSimilar.endDateValue = dateValue ? dateValue.substring( 0, dateValue.indexOf( 'T' ) ) + 'T' + '23:59:59' + dateValue.substring( 19 ) : '';
                catFilterValuesForSearchSimilar.endNumericValue = 0;
                catFilterValuesForSearchSimilar.searchFilterType = 'DateFilter';
                catFilterValuesForSearchSimilar.startDateValue = dateValue ? dateValue.substring( 0, dateValue.indexOf( 'T' ) ) + 'T' + '00:00:00' + dateValue.substring( 19 ) : '';
            ...
            _.forEach( classSearchSimilar[ key ].values, function( fValue ) {
                ...
                if( classSearchSimilar[ key ].formatType === 0 ) {
                    ...
                    catFilterValuesForSearchSimilar.endNumericValue = 0;
                    catFilterValuesForSearchSimilar.startNumericValue = 0;
                    if( classSearchSimilar[ key ] &&
                        classSearchSimilar[ key ].keyLov && classSearchSimilar[ key ].keyLov.keyLOVEntries ) {
    ...
    soaService.post( classifyDefinesService.CLASSIFICATION_SERVICENAME, classifyDefinesService.CLASSIFICATION_OPERATIONNAME, request ).then(
        function( response ) {
            ...
            _.forEach( blockProps, function( prop ) {
                ...
                if( classDesc[ selectedNode.id ].attributes ) {
