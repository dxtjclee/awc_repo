 ...
 */
export let loadDataForTreeKeyLOV = function( treeLoadInput, data, dataProvider, columnConfigsSet, columnProvider ) {
    treeLoadInput = awTableSvc.findTreeLoadInput( arguments );
    if( !treeLoadInput ) {
        var parentNode = awTableTreeSvc.createViewModelTreeNode( 'top', 'unknown', 'top', -1, 0, null );
        treeLoadInput = awTableTreeSvc.createTreeLoadInput( parentNode, 0, null, null,
            40, true );
        treeLoadInput.rootNode = parentNode;
    }
    var children = [];
    ...
    var isTopNode = false;
    isTopNode = treeLoadInput.parentNode.levelNdx === -1;
    if( isTopNode && arr ) {
        counter = 0;
        for( var i = 0; i < arr.length; i++ ) {
            counter += 1;
            buildEntriesTable( arr[ i ], children, treeLoadInput.parentNode.levelNdx, data, counter, columnProvider );
        }
    } else {
        //find the object based upon displayName
        for( var ii = 0; ii < dataProvider.viewModelCollection.loadedVMObjects.length; ii++ ) {
            if( treeLoadInput.parentNode.uid === dataProvider.viewModelCollection.loadedVMObjects[ ii ].uid ) {
                var temp = dataProvider.viewModelCollection.loadedVMObjects[ ii ];
    ...
    if( !columnConfigsSet ) {
        loadColumnsForKeyLOV( dataProvider, data.dataType );
    }
 ...
 */
export let buildEntriesTable = function( obj, table, parentIndx, data, counter, columnProvider ) {
    parentIndx += 1;
    var tempProps = {};
    var vmNode1 = awTableTreeSvc.createViewModelTreeNode(
        '', '',
        '', parentIndx, 0, null );
    ...
    if ( isSupported ) {
        let columnSet = [];
        _.forEach( columnProvider.columns, function( column ) {
            columnSet.push( column.name );
        } );
        buildCell( obj, columnSet, tempProps, vmNode1, counter );
    } else {
    ...
    if ( !isSubMenu ) {
        ...
        isSubMenu = classifyAdminUtil.getObjectAsPerKey( obj, 'IsSubmenu' );
    }
    ...
    if( isSubMenu === true ) {
        ...
        //No guarantee that a submenu will have children. LEaf nodes in >=TC14.3 are called SubMenus without items.
        if( obj.SubMenuItems ) {
            for( var i = 0; i < obj.SubMenuItems.length; i++ ) {
                counter += 1;
                buildEntriesTable( obj.SubMenuItems[ i ], vmNode1.SubMenuItemsNew, parentIndx, data, counter, columnProvider );
            }
        }
    } else {
        vmNode1.isLeaf = true;
...
export let buildCell = function( obj, keyArray, tempProps, vmNode, counter ) {
    ...
    _.forEach( keyArray, function( columnName, index ) {
        ...
        if( index === 0 ) {
            ...
            vmNode.uid = valueObj.toString();
            vmNode.id = valueObj.toString();
        }
    ...
    _.forEach( children, function( child ) {
        _.forEach( child.parents, function( parent ) {
            if ( childParents[ parent.ID ] ) {
                //update parent image
                parent.imageIconUrl = classifyAdminService.addImage( child, type, childParents[parent.ID].ClassType );
            }
        } );
    } );
};
    ...
    if( isSearch && !treeLoadInput.parentNode._expandRequested ) {
        ...
        for( var i = treeLoadInput.startChildNdx; i < children.length; i++ ) {
            ...
            if( childNdx === -1 ) {
                ...
                vmNode = awTableTreeSvc.createViewModelTreeNode(
                    child.id, objType,
 ...
 */
export let loadTreeTableData = function( treeLoadInput, data, dataProviderName, type, isSearch, subPanelContext, classSystem, expand ) {
    ...
    buildTreeTableStructure( deferred, treeLoadInput, data, type, isSearch, isNextPage, subPanelContext, classSystem, expand );
 ...
 * @param {boolean} expand true for expand, false otherwise
 */
function buildTreeTableStructure( deferred, treeLoadInput, data, type, isSearch, isNextPage, subPanelContext, classSystem, expand ) {
    var tempCursorObject = {
        endReached: true,
        startReached: true
    };
    ...
    classifyAdminService.getAdminObjectsForSublocation( data, type, treeLoadInput.parentNode, isSearch, classSystem ).then( function( response ) {
        var children = data.children;
        ...
        var treeLoadResult = awTableTreeSvc.buildTreeLoadResult( treeLoadInput, children1, false, true, tempCursorObject.endReached, null );
        ...
        //reset pwa selection only for fresh load
        if ( !expand ) {
...
/**
 * Used to update the icon for parents. Calls SOA and handles the response
 * @param {*} treeLoadInput Tree load input
 * @param {*} data The view model data object
 * @param {object} type type
 * @param {object} dataProvider data provider
 */
export let loadTreeTableForParents = function( treeLoadInput, data, type, dataProvider ) {
    var deferred = AwPromiseService.instance.defer();
    var isNextPage = false;
    var isTop = treeLoadInput.parentNode.uid === classifyAdminConstants.TOP;
    isNextPage = isTop && treeLoadInput.startChildNdx > 0;
    var tempCursorObject = treeLoadInput.parentNode.cursorObject;
    let parents = data.classParents;
    let objectSet = data.childrenSet;

    classifyAdminService.updateObjectsForParents( data, parents, objectSet, type, treeLoadInput.parentNode ).then( function( response ) {
        var children = data.children;
        var childrenTree = getChildrenForStructure( treeLoadInput, data, type, true, isNextPage );
        children = childrenTree.children;
        var children1 = childrenTree.children1;
        var lastChildNdx = childrenTree.lastChildNdx;
        ...
        treeLoadResult.rootPathNodes = rootPathNodes;
        treeLoadResult.totalChildCount = data.tmpObjectsFound;
        //Update view model to refresh icons
        dataProvider.update( children1, children1.length );
 ...
 * @return {Object} promise for async call
 */
export let loadColumnsForKeyLOV = function( uwDataProvider, type, columnProvider ) {
    var deferred = AwPromiseService.instance.defer();
    ...
    if ( isSupported ) {
        ...
        //Use predefined columns for 14.2
        //check the type
...
export let initialize = function( subPanelContext ) {
    ...
    if ( !adminCtx.classSystem ) {
        //Default to advanced first time
        adminCtx.classSystem = classifyAdminConstants.CLASS_SYSTEM_ADVANCED;
        appCtxService.updateCtx( 'clsAdmin', adminCtx );
        //search state will get updated on second initialize
        return;
    }
    let tmpState = subPanelContext.selectionData.value;
    //reset filters
    let tmpSearchState = subPanelContext.searchState.value;
    if( tmpSearchState.filterMap && tmpSearchState.filterMap.length > 0 ) {
        tmpSearchState.filterMap = [];
        tmpSearchState.clsFilters = {};
        subPanelContext.searchState.update( tmpSearchState );
    }
