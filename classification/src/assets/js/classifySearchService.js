 ...
 *
 * @param {Object} searchInput - parameter for performSearchViewModel5
 * @param {Object} subPanelContext - subpanel context
 * @returns {Object} Returns the response of performSearchViewModel5
 */
            ...
            return soaService.postUnchecked( 'Internal-AWS2-2023-06-Finder', 'performSearchViewModel5', {
                ...
                if ( response.searchFilterMap ) {
                    for ( var i in appCtxService.ctx.preferences.CLS_search_similar_wso_props_enabled ) {
                        ...
                        if ( response.searchFilterMap[key] ) {
                            if ( response.searchFilterMap[key][0].searchFilterType === 'DateFilter' ) {
                                ...
                                if ( selectedItem.props[dateKey] && selectedItem.props[dateKey].dbValues && selectedItem.props[dateKey].dbValues[0] ) {
                                    ...
                                    if ( dateValue ) {
                                        response.searchFilterMap[key][0].startDateValue = dateValue.substring( 0, dateValue.indexOf( 'T' ) ) + 'T' + '00:00:00' + dateValue.substring( 19 );
                                        response.searchFilterMap[key][0].endDateValue = dateValue.substring( 0, dateValue.indexOf( 'T' ) ) + 'T' + '23:59:59' + dateValue.substring( 19 );
                                        response.searchFilterMap[key][0].startEndRange = '';
                                        searchInput.searchFilterMap6[key] = [];
                                        searchInput.searchFilterMap6[key][0] = response.searchFilterMap[key][0];
                                    }
                                }
                            } else {
                                searchInput.searchFilterMap6[key] = response.searchFilterMap[key];
                            }
                        }
                    }
                }
...
export let loadTableData = function( columnConfigInput, saveColumnConfigData, searchInput ) {
    if ( searchInput.searchCriteria && searchInput.searchCriteria.searchString )  {
...
export let getClsSearchCriteria = function( subPanelContext ) {
    ...
    */
    ...
    if ( searchStr === '' && tempClsState.searchStringSecondary !== '' && subPanelContext.selectedNode ) {
...
/**
 * Gets the bulk filter map and returns the appropriate filterMap which can be passed to TcSoaService call for performSearchViewModel5
...
export let copyBulkFiltersToCtx = function( data ) {
    var ctx = appCtxService.getCtx( 'clsLocation' );
    // Deep clone the bulk filter map so that changes between two applyAll/clearAll are not sent to performSearchViewModel5 while changing from List view to table view or viceversa
