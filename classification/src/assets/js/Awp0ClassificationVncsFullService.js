...
/**
 * Removes navigational capabilities when a search is performed in Images tab of Class Navigator.
 * @param {Object} navigateState  the parent uid being navigated to.
 */
export let clearNavigationBtns = function( navigateState ) {
    const tmpState = { ...navigateState.value };
    tmpState.parent = null;
    tmpState.parentDisplayName = null;
    navigateState.update( tmpState );
};
...
let findVncsInfo = function( selectedClass, origSelected, freshRun, navigateState, classifyState, sortOptionVnc, releasesOptions ) {
    ...
    if( selectedClass ) {
        ...
        if( selectedClass.treeLoadResult ) {
            ...
            if( origSelected.selectedNode && origSelected.selected && origSelected.selected.length ) {
                freshRun = false;
            }
...
let updateRoot = ( label, selected, classifyState ) => {
    ...
    if ( classifyState && label && label.uid ) {
        ...
        var newTreeNode = awTableTreeSvc.createViewModelTreeNode( label.uid, label.type, label.displayName, 0, 0, label.iconURL );
