...
import { _localeTextBundle } from 'js/classifyDefinesService';
...
export let updateExternalSelection = function( vmCollection, selectionModel, node, state, gridToExpandId ) {
    if( node ) {
        ...
        if( selectedObject && selectedObject._twistieTitle === _localeTextBundle.SHOW_CHILDREN ) {
 ...
 * @param {Object} navigateState the navigateState object
 * @returns {Object} selectedObject that has been picked
 */
export let updateTreeSelection = function( vmCollection, selectionModel, node, state, deselection, searchString, navigateState ) {
    ...
    if ( node.uid ) {
        ...
        if ( selectedObject && vmCollection.loadedVMObjects.length > 0 && ( selectedObject._twistieTitle === _localeTextBundle.SHOW_CHILDREN || selectedObject._twistieTitle === '' ) ) {
            selectedObject.isExpanded = true;
        ...
        if ( vmCollection.loadedVMObjects.length > 0 && selectedObject._twistieTitle === _localeTextBundle.HIDE_CHILDREN ) {
            ...
            clsTreeSyncUtils.resetTree( vmCollection, selectionModel, state, true, searchString, navigateState );
        ...
        clsTreeSyncUtils.resetTree( vmCollection, selectionModel, state, false, searchString, navigateState );
    }
...
let buildSearchInput = function( treeLoadInput, data, searchString, releasesState, classifyState, sortOption, workspaceObjUid  ) {
    ...
    if( workspaceObjUid && !searchString && sortOption.fielddata.uiValue === '' ) {
...
let getNextLevelChildrenForTree = function( data, providerName ) {
    ...
    return soaPromise && soaPromise.then( ( ) => vncLoadCompleted( data ) );
};
...
let processRejectedResponse = ( deferred, data, treeLoadInput, tempCursorObject ) => {
    ...
    var treeLoadEmptyResult = awTableTreeSvc.buildTreeLoadResult( treeLoadInput, [], false, true, true, null );
    deferred.resolve( {
...
export function convertToVMNodes( rootNodes, treeLoadInput, parentID ) {
    ...
    rootNodes.forEach( ( node, index ) => {
        ...
        let vmNode = awTableTreeSvc.createViewModelTreeNode(
            node.id,
            node.type,
            node.className,
...
function processSearchResponse( deferred, response, data, treeLoadInput, releasesState ) {
    ...
    treeLoadResult = awTableTreeSvc.buildTreeLoadResult(
...
function processDefaultResponse( deferred, response, data, treeLoadInput, searchCriteria ) {
    ...
    for ( var i = 0; i < children.length; i++ ) {
        ...
        if ( children[i].childCount === 0 ) {
            ...
            vmNode._twistieTitle = _localeTextBundle.SHOW_CHILDREN;
            vmNode.isLeaf = false;
        }
    ...
    var treeLoadResult = awTableTreeSvc.buildTreeLoadResult( treeLoadInput, children1, false, true, true, null );
...
let getFirstLevelChildrenForTree = function( data, providerName, selectedNode ) {
    ...
    return soaPromise && soaPromise.then( ( ) => firstLevelLoadCompleted() );
};
...
export let updateNodeData = function( data, searchState, classifyState, selectedNode ) {
    ...
    if ( classifyState ) {
        ...
        if ( tmpState1.editClassCmd && ( tmpState1.editClassCmd.targetClassID !== selectedNode.uid || tmpState1.editClassCmd.alreadySelectedClassNode ) ) {
...
export let makeTreeSelection = function( selectedNode, selectionModel ) {
    ...
    if( selectedNode && !selectedNode.selected && ( !currentSelection || !currentSelection.length ) || currentSelection && selectedNode && currentSelection[0].uid !== selectedNode.uid ) {
        ...
        if ( selectedNode._twistieTitle === _localeTextBundle.SHOW_CHILDREN ) {
