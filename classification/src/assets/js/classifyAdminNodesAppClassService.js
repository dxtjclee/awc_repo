...
export let formatDataAndResponse = function( response, data, tmpState, searchState ) {
    ...
    if( data.classDefinitionMapResponse && tmpState.appClassData !== undefined ) {
        ...
        if( tmpState.propertiesSWA.classSystem === classifyAdminConstants.CLASS_SYSTEM_BASIC ) {
            //SML Node for Older Releases
            _.forEach( classifySvc.ARR_METADATA_NODE_APP_CLASS_JSON_SML_CLASS_PROP, function( key, index ) {
                var value = classifySvc.getPropertyValue(
                    tmpState.appClassData.classesProperties, key );
                let keyDisplay = classifyAdminConstants.ARR_METADATA_NODE_APP_CLASS_JSON_SML_OLDER_RELEASE[ index ];

                if( key === classifySvc.UNCT_CLASS_UNIT_SYSTEM ) {
                    //check value
                    switch ( value ) {
                        case 'metric':
                            value = _metric;
                ...
                var vmoProp = uwPropertyService.createViewModelProperty( key, keyDisplay, '', value.toString(), value.toString() );
                vmoProp.uiValue = value.toString();
                data.nodeClassProperties.push( vmoProp );
            } );
        }
        ...
        var classTypeVal = classifySvc.getPropertyValue(
            tmpState.appClassData.classesProperties, classifySvc.UNCT_CLASS_TYPE );

        if( classTypeVal === classifyAdminConstants.ABSTRACT_CLASS_TYPE ) {
            var classObjType = classifySvc.getPropertyValue(
                tmpState.appClassData.classesProperties, classifySvc.UNCT_CLASS_OBJECT_TYPE );
            if( classObjType !== 'GROUP_NODE' && attributesDefinitions.length === 0 ) {
                tmpState.appClassData.isAbstractNode = true;
            }
        }
        ...
        if( !tmpState.appClassData.isGroupNode ) {
            tmpState.propertiesSWA.appClassProp = true;
        }
        searchState.update( tmpState );
    }
};
