 ...
 */
export let setStandalone = function( selectedNodeRet, classifyState  ) {
...
/**
 * Updates MRU data : Remove deleted standalone id from mru and standalone objcets
 *
 * @param {Object} eventData event data
 * @param {Object} context - search state
 */
export let updateMRUData = function( eventData, context ) {
    const tmpContext = { ...context.value };
    let standaloneUid = eventData.standaloneUid;

    //Remove deleted standalone ico from standAlone objects
    let index = _.findIndex( tmpContext.standAloneObjects, function( object ) {
        return standaloneUid === object.uid;
    } );
    if ( index !== -1 ) {
        tmpContext.standAloneObjects.splice( index, 1 );
    }
    //Remove deleted standalone ico from mru objects
    index = _.findIndex( tmpContext.mruObjects, function( object ) {
        return standaloneUid === object.uid;
    } );
    if ( index !== -1 ) {
        tmpContext.mruObjects.splice( index, 1 );
    }
    if ( tmpContext.mruObjects ) {
        tmpContext.totalFound = tmpContext.mruObjects.length;
        if ( tmpContext.mruObjects.length === 0 ) {
            tmpContext.mruActive = false;
        }
    }
    context.update( tmpContext );
    return tmpContext.mruObjects;
};
