...
/**
 * @module js/IAV1TestMethodTreeTableService
 */
...
function _buildTreeTableStructure( parentNode, deferred, treeLoadInput, columnProviders, parentUid, testMethodTreeTableColumnFilters, testResultColumnFilters, clientScopeURI ) {
    var selectedobj;
    var type = 'IAV0TestRequestRevision';
    var filterOnForTmTree = [];
    if( testMethodTreeTableColumnFilters && testMethodTreeTableColumnFilters.length > 0 ) {
        filterOnForTmTree = testMethodTreeTableColumnFilters;
    } else if( testResultColumnFilters && testResultColumnFilters.length > 0 ) {
        filterOnForTmTree = testResultColumnFilters;
    }
    ...
    if( parentNode.type === 'rootType' ) {
        ...
        columnProviders = filterOnForTmTree;
    } else {
    ...
    return soaSvc.postUnchecked( 'Internal-AWS2-2023-06-Finder', 'performSearchViewModel5', soaInput, policyIOverride ).then(
        function( response ) {
            ...
            if( treeLoadResult.childNodes.length === 0 ) {
                eventBus.publish( 'TMTable.modelObjectsLoaded' );
            }
...
export let loadTestMethodTreeData = function( treeLoadInput, columnProviders, parentUid, testMethodTreeTableColumnFilters, testResultColumnFilters, clientScope ) {
    var clientScopeURI = clientScope.context.vrSublocationState.scopeURI.get( 'IAV0TestRequest' );
    /**
     ...
     */
    _buildTreeTableStructure( treeLoadInput.parentNode, deferred, treeLoadInput, columnProviders, parentUid, testMethodTreeTableColumnFilters, testResultColumnFilters, clientScopeURI );

    return deferred.promise;
};

export let loadTreeTableProperties = function( propertyLoadInput, data, clientScope ) {
    var clientScopeURI =  clientScope.context.vrSublocationState.scopeURI.get( 'IAV0TestRequest' );
    /**
    ...
    if( propertyLoadInput ) {
        return _loadProperties( propertyLoadInput, data, clientScopeURI );
    }

    return AwPromiseService.instance.reject( 'Missing PropertyLoadInput parameter' );
};

function _loadProperties( propertyLoadInput, data, clientScopeURI ) {
    var allChildNodes = [];
    ...
    return tcVmoService.getTableViewModelProperties( allChildNodes, propertyLoadContext ).then(
        function( response ) {
            ...
            if( data.data.panelPinned === true ) {
                eventBus.publish( 'TMTable.setSelection' );
            }
...
function queryColumnConfig( columnConfigUri, clientScopeURI ) {
    // Get Column data
...
export let loadTreeTableColumns = function( dataProvider, data, clientScope ) { // Get the column config
    var clientScopeURI = clientScope.context.vrSublocationState.scopeURI.get( 'IAV0TestRequest' );
    ...
    queryColumnConfig( columnConfigUri, clientScopeURI );
...
export let checkAndReloadTestMethodChart = function( data, chartProp ) {
    if( data.eventData !== undefined ) {
        ...
        if( state === 'saved' && chartProp && chartProp.pieChartData && chartProp.pieChartData.vrTables && chartProp.pieChartData.vrTables.callSoaOnEdit === true ) {
            eventBus.publish( 'TMTable.modelObjectsEdited' );
 ...
 */
export let filterTestMethodTable = function( data, chartProp  ) {
    if( data.eventMap[ 'undefined.selected' ].seriesName === data.i18n.TestMethod ) {
        const vrSubState =  { ...chartProp.value };
        const newChartProp = vrSubState.pieChartData;
        newChartProp.vrTables.testMethodTableProvider.testMethodPieClicked = true;
        ...
        if( data.eventData.label === data.i18n.Pass || data.eventData.label === data.i18n.Fail || data.eventData.label === data.i18n.Blocked || data.eventData.label === data.i18n.Caution ) {
            var testMethodTreeTableColumnFilters = [ {
            ...
            testMethodTreeTableColumnFilters = [ {
        ...
        newChartProp.vrTables.testMethodTableProvider.testMethodTreeTableColumnFilters = testMethodTreeTableColumnFilters;
 ...
 * @param {object} data data
 */
export let displayAllTestMethod = function( data, chartProp ) {
    if( data.eventMap[ 'undefined.unselected' ].seriesName === data.i18n.TestMethod ) {
        if( flagForFilter ) {
            flagForFilter = false;
        } else {
            const vrSubState =  { ...chartProp.value };
            ...
            newChartProp.vrTables.testMethodTableProvider.testMethodTreeTableColumnFilters = [];
            vrSubState.pieChartData = newChartProp;
            chartProp.update && chartProp.update( vrSubState );
            eventBus.publish( 'testMethodTable.plTable.reload' );
        }
...
export let setSelection = function( data ) {
    var dataProvider = data.dataProviders.testMethodTableProvider;
    var selModel = dataProvider.selectionModel;
    var TMTableSelection = data.data.selectedObjects[0];
    selModel.setSelection( TMTableSelection );
};
