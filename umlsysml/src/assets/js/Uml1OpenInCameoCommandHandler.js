...
var exports = {};
var Cameo_MIME_TYPE = 'Sym0Cameo';

/**
 * Note: This module does not return an API object. The API is only available when the service defined this module is
 * injected by AngularJS.
 *
 * @module js/Uml1OpenInCameoCommandHandler
 */
...
export let Uml1OpenInCameoCommandHandler = function( sourceObjects, data, occContext ) {
    var mimeType = null;
    var diagramId = null;
    ...
    soaSvc.post( 'Core-2008-03-Session', 'connect', { featureKey: 'tc_gateway_model', action: 'get' } ).then(
        ( response ) => {
        ...
        .catch(
            ( exception ) => {
                logger.error( 'Failed to get the tc_gateway_model license.' );
                var objectname = '';
                if ( sourceObjects ) {
...
export let downloadPartFile = function( filesUiValues, fileDbValues ) {
    for ( var i = 0; i < filesUiValues.length; i++ ) {
        if ( filesUiValues[i].endsWith( 'prt' ) ) {
            fileMgmtSvc.getFileReadTickets( fileDbValues[i] );
            return;
        }
    }
};
