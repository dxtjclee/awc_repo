...
export let loadTreeTableData = function( searchInput, columnConfigInput, saveColumnConfigData, treeLoadInput ) {
    ...
    //The top node uid for objectNavigationTree is always the opened object(uid)
    searchInput.parentUid = AwStateService.instance.params.uid;
...
export let getDefaultPageSize = function( defaultPageSizePreference ) {
    return AMBreakdownNavigationService.getDefaultPageSize( defaultPageSizePreference );
};
...
export let getSelectedObjectsOnViewModeChange = function( isTreeMode, currentSelections = [] ) {
    ...
    if( isTreeMode ) {
        selections.forEach( ( selection = '' ) => {
            mSelectedNodes.push( getAlternateId( selection.split( ',' )[ 0 ] ) );
        } );
    } else {
        selections.forEach( ( selection = '' ) => {
...
export const updateTreeSelectionInfo = ( { selectionModel, selected } = {}, objNavState, baseSelection = {} ) => {
...
//Add an element to a tree like structure
function addNewlyCreatedElementAndUpdateDataProvider( dataProvider, selectedParentUid, addElementResponse ) {
    ...
    //Set the level and parent uid on the tree object
    ...
    //Update the dataprovider to preserve selection
...
export const handleCdmEventForTree = ( eventData, dataprovider, selectionModel, { dropOnTreeNode, dispatch } = {} ) => {
    ...
    //Fixes pinned selection when an object is added, added due to this PR: 10524013
    if( eventData.createdObjects && eventData.isPinnedFlag ) {
        //create a new viewModel object with the newly created object
        var obj = createVMNodeUsingObjectInfo( eventData.createdObjects[0], 0, selectionModel.selectionData.selected[0].levelNdx + 1 );
        //Add the newly created object and update the dataprovider without reseting the dataprovider
        addNewlyCreatedElementAndUpdateDataProvider( dataprovider,  selectionModel.selectionData.selected[0].uid, obj );
    } else{
        //If we aren't pinned then go on ahead and do what we'd normally do
